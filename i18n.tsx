import React, { createContext, useState, useContext, useEffect, useCallback } from 'react';

const LANGUAGE_STORAGE_KEY = 'veo-app-language';

export type Language = 'en' | 'id' | 'es' | 'zh' | 'hi' | 'ar' | 'pt' | 'bn' | 'ru' | 'ja' | 'de' | 'fr';
type Translations = { [key: string]: string | string[] | Translations };

export const languageMap: { [key in Language]: string } = {
  en: 'English',
  id: 'Bahasa Indonesia',
  es: 'Español',
  zh: '中文 (简体)',
  hi: 'हिन्दी',
  ar: 'العربية',
  pt: 'Português',
  bn: 'বাংলা',
  ru: 'Русский',
  ja: '日本語',
  de: 'Deutsch',
  fr: 'Français',
};

const translations: { [key in Language]: Translations } = {
  // English
  en: {
    appName: "AL ANSHOR VEO GENERATOR",
    appTagline: "Generate stunning videos from text or images.",
    manageStoryApiKeys: "Story API Key",
    manageVideoApiKeys: "Video API Key",
    promptLabel: "Prompt",
    promptPlaceholder: "A majestic lion overlooking the savanna at sunset...",
    promptHint: "You can use plain text or a JSON formatted string.",
    referenceImageLabel: "Reference Image (Optional)",
    uploadFile: "Upload a file",
    dragAndDrop: "or drag and drop",
    fileTypes: "PNG, JPG, GIF up to 10MB",
    generationSettings: "Generation Settings",
    aspectRatioLabel: "Aspect Ratio",
    soundLabel: "Sound",
    enableSound: "Enable sound",
    resolutionLabel: "Resolution",
    videoKeyMissingWarning: "Please add a professional Video API Key to begin.",
    generateButton: "Generate Video",
    generatingButton: "Generating Video...",
    loaderTitle: "Generating Your Video",
    loadingMessages: [ "Initializing VEO model...", "Analyzing your prompt...", "Composing the main scene...", "Generating initial frames...", "Rendering video sequence... this may take a few minutes.", "Upscaling to high resolution...", "Adding sound and final touches...", "Almost there, preparing the final video file." ],
    playerTitle: "Generation Complete!",
    downloadButton: "Download Video",
    storyApiKeyManagerTitle: "Story & Research API Key",
    videoApiKeyManagerTitle: "Professional Video API Key",
    addNewStoryKeyLabel: "Add New Story API Key",
    addNewVideoKeyLabel: "Add New Professional Video API Key",
    apiKeyInputPlaceholder: "Enter your Gemini API Key",
    addKeyButton: "Add Key",
    validatingButton: "Validating...",
    savedStoryKeysLabel: "Saved Story Keys",
    savedVideoKeysLabel: "Saved Professional Video Keys",
    noKeysSaved: "No API Keys saved.",
    addKeyPrompt: "Add a key above to get started.",
    closeButton: "Close",
    errorKeyEmpty: "API Key cannot be empty.",
    errorKeyExists: "This API Key has already been added.",
    errorKeyInvalid: "Invalid API Key or network issue. Please check the key and try again.",
    generationFailed: "Generation Failed",
    alertEnterPrompt: "Please enter a prompt.",
    alertSetStoryApiKey: "Please set an active Story API Key before generating.",
    alertSetVideoApiKey: "Please set an active Professional Video API key before generating a video.",
    backToStoryCreator: "Back to Story Creator",
    storyCreator: {
      newStory: "New Story",
      characterGarage: "Character Garage",
      garageDescription: "Register your favorite toys here to make them the main star!",
      openCharacterWorkshop: "Open Character Workshop",
      garageEmpty: "Your dream garage is empty.",
      directingDesk: "Directing Desk",
      deskDescription: "Additional settings for your story.",
      sceneSet: "Scene Set:",
      locationSet: "Main Location Set:",
      weatherSet: "Weather Set:",
      cameraStyleSet: "Camera Style (POV):",
      narratorLanguageSet: "Narrator Language Set:",
      customLocationPlaceholder: "e.g., Giant Kitchen Diorama",
      customLanguagePlaceholder: "e.g., Sundanese",
      storyEditor: "Story Editor",
      storyboard: "Storyboard",
      needIdea: "Need a Story Idea?",
      ideaDescription: "Use the Smart Director to create a ready-to-air script outline!",
      openSmartDirector: "Open Smart Director",
      storyTitle: "Story Title:",
      storyTitlePlaceholder: "e.g., Rino the Red Racing Car and Goro the Brave Monster Truck",
      storyScript: "Story Script / Summary:",
      storyScriptPlaceholder: "Write a story summary or main idea here. The AI will develop it into professional scenes.",
      sceneCount: "Number of Scenes:",
      createStoryboard: "Create Storyboard!",
      storyboardPlaceholderTitle: "Your Story Results Will Appear Here",
      storyboardPlaceholderDescription: "Click 'Create Storyboard!' to begin.",
      scene: "Scene",
      cinematography: "Cinematography",
      soundEffects: "Sound Effects",
      noSfx: "No SFX suggestions.",
      mixingGuide: "Smart Mixing Guide",
      generateBlueprint: "Design Blueprint",
      generateCinematicPrompt: "Create Cinematic Prompt",
      useThisPrompt: "Generate Video with this Prompt",
      resultBlueprint: "Result: Blueprint Design",
      resultCinematic: "Result: Cinematic Prompt",
      error: "Error",
      confirmNewStoryTitle: "Start a New Story?",
      confirmNewStoryMessage: "All current progress will be deleted. Are you sure?",
      directingOptions: {
        sceneSet: { standard_cinematic: "Standard Cinematic Adventure", epic_destruction: "Epic Destruction (Slow-Motion)", drifting_precision: "Drifting Precision Challenge", },
        locationSet: { standardLandGroup: "🏞️ Standard & Land Sets", natural_outdoor: "Outdoors (Garden/Yard)", kids_bedroom: "Kid's Bedroom", custom_location: "Type your own location...", },
        weatherSet: { sunny: "Bright Sunny", cloudy: "Cloudy", rainy: "Rainy", },
        cameraStyleSet: { standardGroup: "🎥 Standard Styles", standard_cinematic: "Standard Cinematic", fpv_drone_dive: "FPV Drone Dive", },
        narratorLanguageSet: { no_narrator: "Without Narrator", id: "Indonesian", en: "English", custom_language: "Type your own language...", }
      }
    },
    characterWorkshop: {
        titleCreate: "AI Character Workshop",
        titleEdit: "Edit Character",
        step1Title: "Step 1 of 3: Upload an image or describe your toy idea.",
        imagePreview: "Image Preview",
        uploadButton: "Upload Reference Image",
        ideaPlaceholder: "Add notes or additional description (optional if image is uploaded)...",
        designButton: "Design Unique Model",
        designingButton: "Designing...",
        step2Title: "Step 2 of 3: Review and edit the AI-generated model details.",
        brandName: "Fictional Brand Name:",
        modelName: "Specific Model Name:",
        consistencyId: "Consistency ID (Unique Token):",
        consistencyIdHint: "Use this unique ID in every prompt to maintain character consistency.",
        mainMaterial: "Main Material:",
        designLanguage: "Brand Design Language:",
        keyFeatures: "Key Features (Visual DNA):",
        step3Title: "Step 3 of 3: Define this toy's \"Action DNA\".",
        actionDnaSuggestions: "AI Action DNA Suggestions:",
        addCustomAction: "Add Custom Action:",
        customActionPlaceholder: "e.g., Water Skimming",
        addButton: "Add",
        selectedActionDna: "Selected Action DNA:",
        nextButton: "Next",
        backButton: "Back",
        saveButton: "Save Character",
        updateButton: "Update Character",
        alertUploadOrDescribe: "Please upload an image or describe your toy idea.",
        alertRequiredFields: "Brand Name, Model Name, and Consistency ID are required.",
    },
    smartDirector: {
      title: "Smart Director",
      step1Description: "Let's create a ready-to-air script outline! Follow these easy steps.",
      step1Label: "Step 1: Choose Content Format",
      step2Label: "Step 2: Choose Main Character",
      step3Label: "Step 3: Choose Story Theme",
      generateIdeasButton: "Give me 3 Ideas!",
      generatingIdeasButton: "Thinking...",
      step2Title: "Choose Your Favorite Script Idea!",
      tryAgainButton: "↻ Request New Ideas",
      applyIdeaButton: "✅ Use This Idea!",
      cancelButton: "Cancel",
      contentFormats: {
        cinematic_adventure: "Cinematic Adventure & Story",
        product_review: "Product Review",
        unboxing: "Unboxing & First Impressions",
        vs_challenge: "Comparison Video (VS Challenge)",
      },
      characterOptions: {
        random: "Choose Randomly",
        yourGarage: "🚗 Your Garage",
        construction: "🚛 Construction Vehicles",
        beniBulldozer: "Beni the Brave Bulldozer",
      },
      themeOptions: {
        random: "Choose Randomly",
        adventureGroup: "Adventure & Exploration",
        explore_new_area: "Exploring a New Area",
        rescue_mission: "Rescue Mission for Another Toy",
        challengeGroup: "Physical Challenges & Obstacles",
        overcome_obstacle: "Overcoming a Giant Obstacle",
        custom_theme: "Type your own theme...",
      },
      customThemePlaceholder: "e.g., Racing on Planet Mars"
    }
  },
  id: {
    appName: "AL ANSHOR VEO GENERATOR",
    appTagline: "Hasilkan video menakjubkan dari teks atau gambar.",
    manageStoryApiKeys: "Kunci API Cerita",
    manageVideoApiKeys: "Kunci API Video",
    promptLabel: "Prompt",
    promptPlaceholder: "Seekor singa agung memandangi sabana saat matahari terbenam...",
    promptHint: "Anda bisa menggunakan teks biasa atau string format JSON.",
    referenceImageLabel: "Gambar Referensi (Opsional)",
    uploadFile: "Unggah file",
    dragAndDrop: "atau seret dan lepas",
    fileTypes: "PNG, JPG, GIF hingga 10MB",
    generationSettings: "Pengaturan Generasi",
    aspectRatioLabel: "Rasio Aspek",
    soundLabel: "Suara",
    enableSound: "Aktifkan suara",
    resolutionLabel: "Resolusi",
    videoKeyMissingWarning: "Silakan tambah Kunci API Video profesional untuk memulai.",
    generateButton: "Hasilkan Video",
    generatingButton: "Menghasilkan Video...",
    loaderTitle: "Menghasilkan Video Anda",
    loadingMessages: [ "Menginisialisasi model VEO...", "Menganalisis prompt Anda...", "Menyusun adegan utama...", "Menghasilkan bingkai awal...", "Merender urutan video... ini mungkin memakan waktu beberapa menit.", "Meningkatkan skala ke resolusi tinggi...", "Menambahkan suara dan sentuhan akhir...", "Hampir selesai, menyiapkan file video akhir." ],
    playerTitle: "Pembuatan Selesai!",
    downloadButton: "Unduh Video",
    storyApiKeyManagerTitle: "Kunci API Cerita & Riset",
    videoApiKeyManagerTitle: "Kunci API Video Profesional",
    addNewStoryKeyLabel: "Tambah Kunci API Cerita Baru",
    addNewVideoKeyLabel: "Tambah Kunci API Video Profesional Baru",
    apiKeyInputPlaceholder: "Masukkan Kunci API Gemini Anda",
    addKeyButton: "Tambah Kunci",
    validatingButton: "Memvalidasi...",
    savedStoryKeysLabel: "Kunci Cerita Tersimpan",
    savedVideoKeysLabel: "Kunci Video Profesional Tersimpan",
    noKeysSaved: "Tidak ada Kunci API yang tersimpan.",
    addKeyPrompt: "Tambahkan kunci di atas untuk memulai.",
    closeButton: "Tutup",
    errorKeyEmpty: "Kunci API tidak boleh kosong.",
    errorKeyExists: "Kunci API ini sudah ditambahkan.",
    errorKeyInvalid: "Kunci API tidak valid atau masalah jaringan. Harap periksa kunci dan coba lagi.",
    generationFailed: "Pembuatan Gagal",
    alertEnterPrompt: "Silakan masukkan prompt.",
    alertSetStoryApiKey: "Silakan atur Kunci API Cerita yang aktif sebelum melanjutkan.",
    alertSetVideoApiKey: "Silakan atur Kunci API Video Profesional yang aktif sebelum membuat video.",
    backToStoryCreator: "Kembali ke Editor Cerita",
    storyCreator: {
      newStory: "Cerita Baru",
      characterGarage: "Garasi Impianmu",
      garageDescription: "Daftarkan mainan favoritmu di sini untuk menjadikannya bintang utama!",
      openCharacterWorkshop: "Buka Bengkel Karakter",
      garageEmpty: "Garasi impianmu masih kosong.",
      directingDesk: "Meja Bermain",
      deskDescription: "Pengaturan tambahan untuk cerita Anda.",
      sceneSet: "Set Adegan:",
      locationSet: "Set Lokasi Utama:",
      weatherSet: "Set Cuaca:",
      cameraStyleSet: "Gaya Kamera (POV):",
      narratorLanguageSet: "Set Bahasa Narator:",
      customLocationPlaceholder: "Contoh: Diorama Dapur Raksasa",
      customLanguagePlaceholder: "Contoh: Bahasa Sunda",
      storyEditor: "Editor Cerita",
      storyboard: "Papan Cerita",
      needIdea: "Butuh Ide Cerita?",
      ideaDescription: "Gunakan Sutradara Cerdas untuk membuat kerangka naskah yang siap tayang!",
      openSmartDirector: "Buka Sutradara Cerdas",
      storyTitle: "Judul Cerita:",
      storyTitlePlaceholder: "Contoh: Rino si Mobil Balap Merah dan Goro si Truk Monster Pemberani",
      storyScript: "Naskah Cerita / Ringkasan:",
      storyScriptPlaceholder: "Tulis ringkasan cerita atau ide utama di sini. AI akan mengembangkannya menjadi adegan-adegan profesional.",
      sceneCount: "Jumlah Adegan:",
      createStoryboard: "Buat Papan Cerita!",
      storyboardPlaceholderTitle: "Hasil Ceritamu Akan Muncul di Sini",
      storyboardPlaceholderDescription: "Klik \"Buat Papan Cerita!\" untuk memulai.",
      scene: "Adegan",
      cinematography: "Sinematografi",
      soundEffects: "Efek Suara",
      noSfx: "Tidak ada saran SFX.",
      mixingGuide: "Panduan Mixing Cerdas",
      generateBlueprint: "Rancang Blueprint",
      generateCinematicPrompt: "Buat Prompt Sinematik",
      useThisPrompt: "Hasilkan Video dengan Prompt Ini",
      resultBlueprint: "Hasil: Rancang Blueprint",
      resultCinematic: "Hasil: Prompt Sinematik",
      error: "Error",
      confirmNewStoryTitle: "Mulai Cerita Baru?",
      confirmNewStoryMessage: "Semua progres saat ini akan dihapus. Anda yakin?",
      directingOptions: {
        sceneSet: { standard_cinematic: "Petualangan Sinematik Standar", epic_destruction: "Kehancuran Epik (Slow-Motion)", drifting_precision: "Tantangan Presisi Drifting", },
        locationSet: { standardLandGroup: "🏞️ Set Standar & Darat", natural_outdoor: "Luar Ruangan (Taman/Halaman)", kids_bedroom: "Kamar Tidur Anak", custom_location: "Ketik Lokasi Sendiri...", },
        weatherSet: { sunny: "Cerah Terik", cloudy: "Berawan", rainy: "Hujan", },
        cameraStyleSet: { standardGroup: "🎥 Gaya Standar", standard_cinematic: "Sinematik Standar", fpv_drone_dive: "FPV Drone Dive", },
        narratorLanguageSet: { no_narrator: "Tanpa Narator", id: "Bahasa Indonesia", en: "English (Bahasa Inggris)", custom_language: "Ketik Bahasa Sendiri...", }
      }
    },
    characterWorkshop: {
        titleCreate: "Bengkel Karakter AI",
        titleEdit: "Edit Karakter",
        step1Title: "Langkah 1 dari 3: Unggah gambar atau jelaskan ide mainan Anda.",
        imagePreview: "Pratinjau Gambar",
        uploadButton: "Unggah Gambar Referensi",
        ideaPlaceholder: "Tambahkan catatan atau deskripsi tambahan (opsional jika gambar diunggah)...",
        designButton: "Desain Model Unik",
        designingButton: "Mendesain...",
        step2Title: "Langkah 2 dari 3: Tinjau dan sunting detail model yang dibuat AI.",
        brandName: "Nama Merek Fiktif:",
        modelName: "Nama Model Spesifik:",
        consistencyId: "ID Konsistensi (Token Unik):",
        consistencyIdHint: "Gunakan ID unik ini di setiap prompt untuk menjaga konsistensi karakter.",
        mainMaterial: "Material Utama:",
        designLanguage: "Bahasa Desain Merek:",
        keyFeatures: "Fitur Kunci (DNA Visual):",
        step3Title: "Langkah 3 dari 3: Definisikan \"DNA Aksi\" mainan ini.",
        actionDnaSuggestions: "Saran DNA Aksi dari AI:",
        addCustomAction: "Tambah Aksi Kustom:",
        customActionPlaceholder: "Contoh: Skimming Air",
        addButton: "Tambah",
        selectedActionDna: "DNA Aksi Terpilih:",
        nextButton: "Lanjut",
        backButton: "Kembali",
        saveButton: "Simpan Karakter",
        updateButton: "Perbarui Karakter",
        alertUploadOrDescribe: "Harap unggah gambar atau jelaskan ide mainan Anda.",
        alertRequiredFields: "Nama Merek, Nama Model, dan ID Konsistensi wajib diisi.",
    },
    smartDirector: {
      title: "Sutradara Cerdas",
      step1Description: "Ayo kita buat kerangka naskah yang siap tayang! Ikuti langkah-langkah mudah ini.",
      step1Label: "Langkah 1: Pilih Format Konten",
      step2Label: "Langkah 2: Pilih Karakter Utama",
      step3Label: "Langkah 3: Pilih Tema Cerita",
      generateIdeasButton: "Berikan 3 Ide!",
      generatingIdeasButton: "Mencari Ide...",
      step2Title: "Pilih Ide Naskah Favoritmu!",
      tryAgainButton: "↻ Minta Ide Baru",
      applyIdeaButton: "✅ Pakai Ide Ini!",
      cancelButton: "Batal",
      contentFormats: {
        cinematic_adventure: "Petualangan & Cerita Sinematik",
        product_review: "Ulasan Produk (Review)",
        unboxing: "Unboxing & Kesan Pertama",
        vs_challenge: "Video Perbandingan (VS Challenge)",
      },
      characterOptions: {
        random: "Pilihkan Secara Acak",
        yourGarage: "🚗 Garasi Anda",
        construction: "🚛 Kendaraan Konstruksi",
        beniBulldozer: "Beni si Buldoser Pemberani",
      },
      themeOptions: {
        random: "Pilihkan Secara Acak",
        adventureGroup: "Petualangan & Eksplorasi",
        explore_new_area: "Menjelajahi Area Baru",
        rescue_mission: "Misi Penyelamatan Mainan Lain",
        challengeGroup: "Tantangan & Rintangan Fisik",
        overcome_obstacle: "Mengatasi Rintangan Raksasa",
        custom_theme: "Ketik Tema Sendiri...",
      },
      customThemePlaceholder: "Contoh: Balapan di Planet Mars"
    }
  },
  // Spanish
  es: {
    appName: "GENERADOR VEO AL ANSHOR",
    appTagline: "Genera videos impresionantes a partir de texto o imágenes.",
    manageStoryApiKeys: "Clave API de Historia",
    manageVideoApiKeys: "Clave API de Video",
    promptLabel: "Prompt",
    promptPlaceholder: "Un majestuoso león contemplando la sabana al atardecer...",
    promptHint: "Puedes usar texto plano o una cadena en formato JSON.",
    referenceImageLabel: "Imagen de Referencia (Opcional)",
    uploadFile: "Subir un archivo",
    dragAndDrop: "o arrastrar y soltar",
    fileTypes: "PNG, JPG, GIF hasta 10MB",
    generationSettings: "Ajustes de Generación",
    aspectRatioLabel: "Relación de Aspecto",
    soundLabel: "Sonido",
    enableSound: "Habilitar sonido",
    resolutionLabel: "Resolución",
    videoKeyMissingWarning: "Por favor, añade una Clave API de Video profesional para comenzar.",
    generateButton: "Generar Video",
    generatingButton: "Generando Video...",
    loaderTitle: "Generando Tu Video",
    loadingMessages: [ "Inicializando modelo VEO...", "Analizando tu prompt...", "Componiendo la escena principal...", "Generando fotogramas iniciales...", "Renderizando secuencia de video... esto puede tardar unos minutos.", "Ampliando a alta resolución...", "Añadiendo sonido y toques finales...", "Casi listo, preparando el archivo de video final." ],
    playerTitle: "¡Generación Completa!",
    downloadButton: "Descargar Video",
    storyApiKeyManagerTitle: "Clave API de Historia e Investigación",
    videoApiKeyManagerTitle: "Clave API de Video Profesional",
    addNewStoryKeyLabel: "Añadir Nueva Clave API de Historia",
    addNewVideoKeyLabel: "Añadir Nueva Clave API de Video Profesional",
    apiKeyInputPlaceholder: "Introduce tu Clave API de Gemini",
    addKeyButton: "Añadir Clave",
    validatingButton: "Validando...",
    savedStoryKeysLabel: "Claves de Historia Guardadas",
    savedVideoKeysLabel: "Claves de Video Profesional Guardadas",
    noKeysSaved: "No hay Claves API guardadas.",
    addKeyPrompt: "Añade una clave arriba para empezar.",
    closeButton: "Cerrar",
    errorKeyEmpty: "La Clave API no puede estar vacía.",
    errorKeyExists: "Esta Clave API ya ha sido añadida.",
    errorKeyInvalid: "Clave API inválida o problema de red. Por favor, comprueba la clave e inténtalo de nuevo.",
    generationFailed: "Generación Fallida",
    alertEnterPrompt: "Por favor, introduce un prompt.",
    alertSetStoryApiKey: "Por favor, establece una clave API de Historia activa antes de generar.",
    alertSetVideoApiKey: "Por favor, establece una clave API de Video Profesional activa antes de generar un video.",
    backToStoryCreator: "Volver al Creador de Historias",
    storyCreator: {
      newStory: "Nueva Historia",
      characterGarage: "Garaje de Personajes",
      garageDescription: "¡Registra tus juguetes favoritos aquí para convertirlos en la estrella principal!",
      openCharacterWorkshop: "Abrir Taller de Personajes",
      garageEmpty: "Tu garaje de ensueño está vacío.",
      directingDesk: "Mesa de Dirección",
      deskDescription: "Ajustes adicionales para tu historia.",
      sceneSet: "Set de Escena:",
      locationSet: "Set de Ubicación Principal:",
      weatherSet: "Set de Clima:",
      cameraStyleSet: "Estilo de Cámara (POV):",
      narratorLanguageSet: "Idioma del Narrador:",
      customLocationPlaceholder: "Ej: Diorama de Cocina Gigante",
      customLanguagePlaceholder: "Ej: Catalán",
      storyEditor: "Editor de Historias",
      storyboard: "Guion Gráfico",
      needIdea: "¿Necesitas una idea para una historia?",
      ideaDescription: "¡Usa el Director Inteligente para crear un borrador de guion listo para usar!",
      openSmartDirector: "Abrir Director Inteligente",
      storyTitle: "Título de la Historia:",
      storyTitlePlaceholder: "Ej: Rino el Coche de Carreras Rojo y Goro el Valiente Camión Monstruo",
      storyScript: "Guion / Resumen de la Historia:",
      storyScriptPlaceholder: "Escribe un resumen de la historia o la idea principal aquí. La IA lo desarrollará en escenas profesionales.",
      sceneCount: "Número de Escenas:",
      createStoryboard: "¡Crear Guion Gráfico!",
      storyboardPlaceholderTitle: "Los Resultados de tu Historia Aparecerán Aquí",
      storyboardPlaceholderDescription: "Haz clic en '¡Crear Guion Gráfico!' para empezar.",
      scene: "Escena",
      cinematography: "Cinematografía",
      soundEffects: "Efectos de Sonido",
      noSfx: "No hay sugerencias de SFX.",
      mixingGuide: "Guía de Mezcla Inteligente",
      generateBlueprint: "Diseñar Prototipo",
      generateCinematicPrompt: "Crear Prompt Cinemático",
      useThisPrompt: "Generar Video con este Prompt",
      resultBlueprint: "Resultado: Diseño de Prototipo",
      resultCinematic: "Resultado: Prompt Cinemático",
      error: "Error",
      confirmNewStoryTitle: "¿Empezar una Nueva Historia?",
      confirmNewStoryMessage: "Todo el progreso actual será eliminado. ¿Estás seguro?",
      directingOptions: {
        sceneSet: { standard_cinematic: "Aventura Cinemática Estándar", epic_destruction: "Destrucción Épica (Cámara Lenta)", drifting_precision: "Desafío de Precisión de Derrape" },
        locationSet: { standardLandGroup: "🏞️ Sets Estándar y Terrestres", natural_outdoor: "Exteriores (Jardín/Patio)", kids_bedroom: "Habitación de Niño", custom_location: "Escribe tu propia ubicación..." },
        weatherSet: { sunny: "Soleado Brillante", cloudy: "Nublado", rainy: "Lluvioso" },
        cameraStyleSet: { standardGroup: "🎥 Estilos Estándar", standard_cinematic: "Cinemático Estándar", fpv_drone_dive: "Picado de Dron FPV" },
        narratorLanguageSet: { no_narrator: "Sin Narrador", id: "Indonesio", en: "Inglés", custom_language: "Escribe tu propio idioma..." }
      }
    },
    characterWorkshop: {
      titleCreate: "Taller de Personajes con IA",
      titleEdit: "Editar Personaje",
      step1Title: "Paso 1 de 3: Sube una imagen o describe tu idea de juguete.",
      imagePreview: "Vista Previa de Imagen",
      uploadButton: "Subir Imagen de Referencia",
      ideaPlaceholder: "Añade notas o descripción adicional (opcional si se sube imagen)...",
      designButton: "Diseñar Modelo Único",
      designingButton: "Diseñando...",
      step2Title: "Paso 2 de 3: Revisa y edita los detalles del modelo generados por la IA.",
      brandName: "Nombre de Marca Ficticio:",
      modelName: "Nombre de Modelo Específico:",
      consistencyId: "ID de Consistencia (Token Único):",
      consistencyIdHint: "Usa este ID único en cada prompt para mantener la consistencia del personaje.",
      mainMaterial: "Material Principal:",
      designLanguage: "Lenguaje de Diseño de la Marca:",
      keyFeatures: "Características Clave (ADN Visual):",
      step3Title: "Paso 3 de 3: Define el \"ADN de Acción\" de este juguete.",
      actionDnaSuggestions: "Sugerencias de ADN de Acción de la IA:",
      addCustomAction: "Añadir Acción Personalizada:",
      customActionPlaceholder: "Ej: Deslizamiento sobre agua",
      addButton: "Añadir",
      selectedActionDna: "ADN de Acción Seleccionado:",
      nextButton: "Siguiente",
      backButton: "Atrás",
      saveButton: "Guardar Personaje",
      updateButton: "Actualizar Personaje",
      alertUploadOrDescribe: "Por favor, sube una imagen o describe tu idea de juguete.",
      alertRequiredFields: "El Nombre de Marca, Nombre de Modelo y ID de Consistencia son obligatorios."
    },
    smartDirector: {
      title: "Director Inteligente",
      step1Description: "¡Vamos a crear un borrador de guion listo para usar! Sigue estos sencillos pasos.",
      step1Label: "Paso 1: Elige el formato del contenido",
      step2Label: "Paso 2: Elige el personaje principal",
      step3Label: "Paso 3: Elige el tema de la historia",
      generateIdeasButton: "¡Dame 3 ideas!",
      generatingIdeasButton: "Pensando...",
      step2Title: "¡Elige tu idea de guion favorita!",
      tryAgainButton: "↻ Solicitar nuevas ideas",
      applyIdeaButton: "✅ ¡Usar esta idea!",
      cancelButton: "Cancelar",
      contentFormats: {
        cinematic_adventure: "Aventura y historia cinemática",
        product_review: "Reseña de producto",
        unboxing: "Unboxing y primeras impresiones",
        vs_challenge: "Vídeo de comparación (desafío VS)",
      },
      characterOptions: {
        random: "Elegir al azar",
        yourGarage: "🚗 Tu garaje",
        construction: "🚛 Vehículos de construcción",
        beniBulldozer: "Beni el Bulldozer Valiente",
      },
      themeOptions: {
        random: "Elegir al azar",
        adventureGroup: "Aventura y exploración",
        explore_new_area: "Explorando una nueva área",
        rescue_mission: "Misión de rescate para otro juguete",
        challengeGroup: "Desafíos y obstáculos físicos",
        overcome_obstacle: "Superando un obstáculo gigante",
        custom_theme: "Escribe tu propio tema...",
      },
      customThemePlaceholder: "Ej: Carreras en el planeta Marte"
    }
  },
  zh: {
    appName: "AL ANSHOR VEO 生成器",
    appTagline: "通过文本或图像生成令人惊叹的视频。",
    manageStoryApiKeys: "故事API密钥",
    manageVideoApiKeys: "视频API密钥",
    promptLabel: "提示",
    promptPlaceholder: "一只雄伟的狮子在日落时俯瞰着大草原...",
    promptHint: "您可以使用纯文本或JSON格式的字符串。",
    referenceImageLabel: "参考图像（可选）",
    uploadFile: "上传文件",
    dragAndDrop: "或拖放",
    fileTypes: "PNG, JPG, GIF, 最大10MB",
    generationSettings: "生成设置",
    aspectRatioLabel: "宽高比",
    soundLabel: "声音",
    enableSound: "启用声音",
    resolutionLabel: "分辨率",
    videoKeyMissingWarning: "请添加一个专业的视频API密钥以开始。",
    generateButton: "生成视频",
    generatingButton: "正在生成视频...",
    loaderTitle: "正在生成您的视频",
    loadingMessages: [ "正在初始化VEO模型...", "正在分析您的提示...", "正在构图主场景...", "正在生成初始帧...", "正在渲染视频序列...这可能需要几分钟。", "正在放大到高分辨率...", "正在添加声音和最后润色...", "即将完成，正在准备最终视频文件。" ],
    playerTitle: "生成完成！",
    downloadButton: "下载视频",
    storyApiKeyManagerTitle: "故事与研究API密钥",
    videoApiKeyManagerTitle: "专业视频API密钥",
    addNewStoryKeyLabel: "添加新的故事API密钥",
    addNewVideoKeyLabel: "添加新的专业视频API密钥",
    apiKeyInputPlaceholder: "输入您的 Gemini API 密钥",
    addKeyButton: "添加密钥",
    validatingButton: "验证中...",
    savedStoryKeysLabel: "已存故事密钥",
    savedVideoKeysLabel: "已存专业视频密钥",
    noKeysSaved: "未保存任何 API 密钥。",
    addKeyPrompt: "在上方添加一个密钥以开始使用。",
    closeButton: "关闭",
    errorKeyEmpty: "API 密钥不能为空。",
    errorKeyExists: "此 API 密钥已添加。",
    errorKeyInvalid: "API 密钥无效或网络问题。请检查密钥并重试。",
    generationFailed: "生成失败",
    alertEnterPrompt: "请输入提示。",
    alertSetStoryApiKey: "生成前，请设置有效的故事API密钥。",
    alertSetVideoApiKey: "生成视频前，请设置有效的专业视频API密钥。",
    backToStoryCreator: "返回故事创建器",
    storyCreator: {
      newStory: "新故事",
      characterGarage: "角色车库",
      garageDescription: "在这里注册你最喜欢的玩具，让它们成为主角！",
      openCharacterWorkshop: "打开角色工作室",
      garageEmpty: "你的梦想车库是空的。",
      directingDesk: "导演台",
      deskDescription: "为您的故事提供额外设置。",
      sceneSet: "场景设置:",
      locationSet: "主要地点设置:",
      weatherSet: "天气设置:",
      cameraStyleSet: "相机风格 (POV):",
      narratorLanguageSet: "旁白语言设置:",
      customLocationPlaceholder: "例如：巨型厨房立体模型",
      customLanguagePlaceholder: "例如：粤语",
      storyEditor: "故事编辑器",
      storyboard: "故事板",
      needIdea: "需要故事灵感吗？",
      ideaDescription: "使用智能导筒创建一个准备播出的剧本大纲！",
      openSmartDirector: "打开智能导筒",
      storyTitle: "故事标题:",
      storyTitlePlaceholder: "例如：红色赛车里诺和勇敢的怪物卡车戈罗",
      storyScript: "故事剧本/摘要:",
      storyScriptPlaceholder: "在这里写下故事摘要或主要思想。AI会将其发展成专业的场景。",
      sceneCount: "场景数量:",
      createStoryboard: "创建故事板！",
      storyboardPlaceholderTitle: "您的故事结果将显示在此处",
      storyboardPlaceholderDescription: "点击“创建故事板！”开始。",
      scene: "场景",
      cinematography: "摄影",
      soundEffects: "音效",
      noSfx: "无音效建议。",
      mixingGuide: "智能混音指南",
      generateBlueprint: "设计蓝图",
      generateCinematicPrompt: "创建电影级提示",
      useThisPrompt: "使用此提示生成视频",
      resultBlueprint: "结果：蓝图设计",
      resultCinematic: "结果：电影级提示",
      error: "错误",
      confirmNewStoryTitle: "开始一个新故事？",
      confirmNewStoryMessage: "所有当前进度将被删除。您确定吗？",
      directingOptions: {
        sceneSet: { standard_cinematic: "标准电影冒险", epic_destruction: "史诗级破坏（慢动作）", drifting_precision: "漂移精度挑战" },
        locationSet: { standardLandGroup: "🏞️ 标准与陆地场景", natural_outdoor: "户外（花园/院子）", kids_bedroom: "儿童卧室", custom_location: "输入您自己的地点..." },
        weatherSet: { sunny: "晴朗", cloudy: "多云", rainy: "雨天" },
        cameraStyleSet: { standardGroup: "🎥 标准风格", standard_cinematic: "标准电影", fpv_drone_dive: "FPV无人机俯冲" },
        narratorLanguageSet: { no_narrator: "无旁白", id: "印尼语", en: "英语", custom_language: "输入您自己的语言..." }
      }
    },
    characterWorkshop: {
      titleCreate: "AI 角色工作室",
      titleEdit: "编辑角色",
      step1Title: "第1步（共3步）：上传图片或描述您的玩具创意。",
      imagePreview: "图片预览",
      uploadButton: "上传参考图片",
      ideaPlaceholder: "添加注释或其他描述（如果上传了图片，则为可选项）...",
      designButton: "设计独特模型",
      designingButton: "设计中...",
      step2Title: "第2步（共3步）：审查和编辑AI生成的模型细节。",
      brandName: "虚构品牌名称:",
      modelName: "具体型号名称:",
      consistencyId: "一致性ID（唯一令牌）:",
      consistencyIdHint: "在每个提示中使用此唯一ID以保持角色一致性。",
      mainMaterial: "主要材料:",
      designLanguage: "品牌设计语言:",
      keyFeatures: "关键特征（视觉DNA）:",
      step3Title: "第3步（共3步）：定义此玩具的“动作DNA”。",
      actionDnaSuggestions: "AI 动作DNA建议:",
      addCustomAction: "添加自定义动作:",
      customActionPlaceholder: "例如：水上漂",
      addButton: "添加",
      selectedActionDna: "已选动作DNA:",
      nextButton: "下一步",
      backButton: "上一步",
      saveButton: "保存角色",
      updateButton: "更新角色",
      alertUploadOrDescribe: "请上传图片或描述您的玩具创意。",
      alertRequiredFields: "品牌名称、型号名称和一致性ID是必填项。"
    },
    smartDirector: {
      title: "智能导筒",
      step1Description: "让我们创建一个准备播出的剧本大纲！请遵循以下简单步骤。",
      step1Label: "第1步：选择内容格式",
      step2Label: "第2步：选择主角",
      step3Label: "第3步：选择故事主题",
      generateIdeasButton: "给我3个主意！",
      generatingIdeasButton: "思考中...",
      step2Title: "选择您最喜欢的剧本主意！",
      tryAgainButton: "↻ 请求新主意",
      applyIdeaButton: "✅ 使用这个主意！",
      cancelButton: "取消",
      contentFormats: {
        cinematic_adventure: "电影冒险与故事",
        product_review: "产品评测",
        unboxing: "开箱和第一印象",
        vs_challenge: "对比视频（VS挑战）",
      },
      characterOptions: {
        random: "随机选择",
        yourGarage: "🚗 您的车库",
        construction: "🚛 工程车辆",
        beniBulldozer: "勇敢的推土机贝尼",
      },
      themeOptions: {
        random: "随机选择",
        adventureGroup: "冒险与探索",
        explore_new_area: "探索新区域",
        rescue_mission: "救援另一个玩具的任务",
        challengeGroup: "物理挑战与障碍",
        overcome_obstacle: "克服巨大障碍",
        custom_theme: "输入您自己的主题...",
      },
      customThemePlaceholder: "例如：在火星上赛车"
    }
  },
  hi: {
    appName: "AL ANSHOR VEO जेनरेटर",
    appTagline: "टेक्स्ट या छवियों से शानदार वीडियो बनाएं।",
    manageStoryApiKeys: "कहानी API कुंजी",
    manageVideoApiKeys: "वीडियो API कुंजी",
    promptLabel: "प्रॉम्प्ट",
    promptPlaceholder: "सूर्यास्त के समय सवाना को देखता हुआ एक शानदार शेर...",
    promptHint: "आप सादा पाठ या JSON स्वरूपित स्ट्रिंग का उपयोग कर सकते हैं।",
    referenceImageLabel: "संदर्भ छवि (वैकल्पिक)",
    uploadFile: "फ़ाइल अपलोड करें",
    dragAndDrop: "या खींचें और छोड़ें",
    fileTypes: "PNG, JPG, GIF 10MB तक",
    generationSettings: "उत्पादन सेटिंग्स",
    aspectRatioLabel: "पहलू अनुपात",
    soundLabel: "ध्वनि",
    enableSound: "ध्वनि सक्षम करें",
    resolutionLabel: "रिज़ॉल्यूशन",
    videoKeyMissingWarning: "शुरू करने के लिए कृपया एक पेशेवर वीडियो API कुंजी जोड़ें।",
    generateButton: "वीडियो बनाएं",
    generatingButton: "वीडियो बना रहा है...",
    loaderTitle: "आपका वीडियो बना रहा है",
    loadingMessages: [ "VEO मॉडल प्रारंभ हो रहा है...", "आपके प्रॉम्प्ट का विश्लेषण हो रहा है...", "मुख्य दृश्य की रचना हो रही है...", "प्रारंभिक फ़्रेम उत्पन्न हो रहे हैं...", "वीडियो अनुक्रम प्रस्तुत हो रहा है... इसमें कुछ मिनट लग सकते हैं।", "उच्च रिज़ॉल्यूशन में अपस्केलिंग...", "ध्वनि और अंतिम स्पर्श जोड़ना...", "लगभग हो गया, अंतिम वीडियो फ़ाइल तैयार हो रही है।" ],
    playerTitle: "निर्माण पूर्ण!",
    downloadButton: "वीडियो डाउनलोड करें",
    storyApiKeyManagerTitle: "कहानी और अनुसंधान API कुंजी",
    videoApiKeyManagerTitle: "पेशेवर वीडियो API कुंजी",
    addNewStoryKeyLabel: "नई कहानी API कुंजी जोड़ें",
    addNewVideoKeyLabel: "नई पेशेवर वीडियो API कुंजी जोड़ें",
    apiKeyInputPlaceholder: "अपनी जेमिनी API कुंजी दर्ज करें",
    addKeyButton: "कुंजी जोड़ें",
    validatingButton: "सत्यापन हो रहा है...",
    savedStoryKeysLabel: "सहेजी गई कहानी कुंजियाँ",
    savedVideoKeysLabel: "सहेजी गई पेशेवर वीडियो कुंजियाँ",
    noKeysSaved: "कोई API कुंजी सहेजी नहीं गई।",
    addKeyPrompt: "शुरू करने के लिए ऊपर एक कुंजी जोड़ें।",
    closeButton: "बंद करें",
    errorKeyEmpty: "API कुंजी खाली नहीं हो सकती।",
    errorKeyExists: "यह API कुंजी पहले ही जोड़ी जा चुकी है।",
    errorKeyInvalid: "अमान्य API कुंजी या नेटवर्क समस्या। कृपया कुंजी जांचें और पुनः प्रयास करें।",
    generationFailed: "निर्माण विफल",
    alertEnterPrompt: "कृपया एक प्रॉम्प्ट दर्ज करें।",
    alertSetStoryApiKey: "उत्पन्न करने से पहले कृपया एक सक्रिय कहानी API कुंजी सेट करें।",
    alertSetVideoApiKey: "वीडियो बनाने से पहले कृपया एक सक्रिय पेशेवर वीडियो API कुंजी सेट करें।",
    backToStoryCreator: "कहानी निर्माता पर वापस जाएं",
    storyCreator: {
      newStory: "नई कहानी",
      characterGarage: "चरित्र गैराज",
      garageDescription: "अपने पसंदीदा खिलौनों को यहाँ पंजीकृत करें ताकि वे मुख्य सितारा बन सकें!",
      openCharacterWorkshop: "चरित्र कार्यशाला खोलें",
      garageEmpty: "आपका सपनों का गैराज खाली है।",
      directingDesk: "निर्देशन डेस्क",
      deskDescription: "आपकी कहानी के लिए अतिरिक्त सेटिंग्स।",
      sceneSet: "दृश्य सेट:",
      locationSet: "मुख्य स्थान सेट:",
      weatherSet: "मौसम सेट:",
      cameraStyleSet: "कैमरा शैली (POV):",
      narratorLanguageSet: "कथावाचक भाषा सेट:",
      customLocationPlaceholder: "जैसे, विशाल रसोई डायोरमा",
      customLanguagePlaceholder: "जैसे, पंजाबी",
      storyEditor: "कहानी संपादक",
      storyboard: "स्टोरीबोर्ड",
      needIdea: "कहानी का विचार चाहिए?",
      ideaDescription: "प्रसारण के लिए तैयार स्क्रिप्ट की रूपरेखा बनाने के लिए स्मार्ट डायरेक्टर का उपयोग करें!",
      openSmartDirector: "स्मार्ट डायरेक्टर खोलें",
      storyTitle: "कहानी का शीर्षक:",
      storyTitlePlaceholder: "जैसे, लाल रेसिंग कार रीनो और बहादुर मॉन्स्टर ट्रक गोरो",
      storyScript: "कहानी स्क्रिप्ट / सारांश:",
      storyScriptPlaceholder: "यहाँ एक कहानी का सारांश या मुख्य विचार लिखें। AI इसे पेशेवर दृश्यों में विकसित करेगा।",
      sceneCount: "दृश्यों की संख्या:",
      createStoryboard: "स्टोरीबोर्ड बनाएं!",
      storyboardPlaceholderTitle: "आपकी कहानी के परिणाम यहाँ दिखाई देंगे",
      storyboardPlaceholderDescription: "शुरू करने के लिए 'स्टोरीबोर्ड बनाएं!' पर क्लिक करें।",
      scene: "दृश्य",
      cinematography: "सिनेमैटोग्राफी",
      soundEffects: "ध्वनि प्रभाव",
      noSfx: "कोई SFX सुझाव नहीं।",
      mixingGuide: "स्मार्ट मिक्सिंग गाइड",
      generateBlueprint: "ब्लूप्रिंट डिज़ाइन करें",
      generateCinematicPrompt: "सिनेमैटिक प्रॉम्प्ट बनाएं",
      useThisPrompt: "इस प्रॉम्प्ट के साथ वीडियो बनाएं",
      resultBlueprint: "परिणाम: ब्लूप्रिंट डिज़ाइन",
      resultCinematic: "परिणाम: सिनेमैटिक प्रॉम्प्ट",
      error: "त्रुटि",
      confirmNewStoryTitle: "एक नई कहानी शुरू करें?",
      confirmNewStoryMessage: "सभी वर्तमान प्रगति हटा दी जाएगी। क्या आप निश्चित हैं?",
      directingOptions: {
        sceneSet: { standard_cinematic: "मानक सिनेमैटिक साहसिक", epic_destruction: "महाकाव्य विनाश (धीमी गति)", drifting_precision: "ड्रिफ्टिंग परिशुद्धता चुनौती" },
        locationSet: { standardLandGroup: "🏞️ मानक और भूमि सेट", natural_outdoor: "बाहर (बगीचा/आंगन)", kids_bedroom: "बच्चों का शयनकक्ष", custom_location: "अपना स्थान टाइप करें..." },
        weatherSet: { sunny: "तेज धूप", cloudy: "बादल छाए हुए", rainy: "बरसात" },
        cameraStyleSet: { standardGroup: "🎥 मानक शैलियाँ", standard_cinematic: "मानक सिनेमैटिक", fpv_drone_dive: "FPV ड्रोन डाइव" },
        narratorLanguageSet: { no_narrator: "बिना कथावाचक के", id: "इंडोनेशियाई", en: "अंग्रेज़ी", custom_language: "अपनी भाषा टाइप करें..." }
      }
    },
    characterWorkshop: {
      titleCreate: "AI चरित्र कार्यशाला",
      titleEdit: "चरित्र संपादित करें",
      step1Title: "चरण 1/3: एक छवि अपलोड करें या अपने खिलौने का विचार बताएं।",
      imagePreview: "छवि पूर्वावलोकन",
      uploadButton: "संदर्भ छवि अपलोड करें",
      ideaPlaceholder: "नोट्स या अतिरिक्त विवरण जोड़ें (यदि छवि अपलोड की गई है तो वैकल्पिक)...",
      designButton: "अद्वितीय मॉडल डिज़ाइन करें",
      designingButton: "डिज़ाइन हो रहा है...",
      step2Title: "चरण 2/3: AI-जनित मॉडल विवरणों की समीक्षा और संपादन करें।",
      brandName: "काल्पनिक ब्रांड नाम:",
      modelName: "विशिष्ट मॉडल नाम:",
      consistencyId: "संगति आईडी (अद्वितीय टोकन):",
      consistencyIdHint: "चरित्र संगति बनाए रखने के लिए हर प्रॉम्प्ट में इस अद्वितीय आईडी का उपयोग करें।",
      mainMaterial: "मुख्य सामग्री:",
      designLanguage: "ब्रांड डिज़ाइन भाषा:",
      keyFeatures: "मुख्य विशेषताएँ (दृश्य डीएनए):",
      step3Title: "चरण 3/3: इस खिलौने के \"एक्शन डीएनए\" को परिभाषित करें।",
      actionDnaSuggestions: "AI एक्शन डीएनए सुझाव:",
      addCustomAction: "कस्टम एक्शन जोड़ें:",
      customActionPlaceholder: "जैसे, पानी पर फिसलना",
      addButton: "जोड़ें",
      selectedActionDna: "चयनित एक्शन डीएनए:",
      nextButton: "अगला",
      backButton: "पिछला",
      saveButton: "चरित्र सहेजें",
      updateButton: "चरित्र अपडेट करें",
      alertUploadOrDescribe: "कृपया एक छवि अपलोड करें या अपने खिलौने का विचार बताएं।",
      alertRequiredFields: "ब्रांड नाम, मॉडल नाम, और संगति आईडी आवश्यक हैं।"
    },
    smartDirector: {
      title: "स्मार्ट डायरेक्टर",
      step1Description: "आइए एक प्रसारण के लिए तैयार स्क्रिप्ट की रूपरेखा बनाएं! इन आसान चरणों का पालन करें।",
      step1Label: "चरण 1: सामग्री प्रारूप चुनें",
      step2Label: "चरण 2: मुख्य चरित्र चुनें",
      step3Label: "चरण 3: कहानी का विषय चुनें",
      generateIdeasButton: "मुझे 3 विचार दें!",
      generatingIdeasButton: "सोच रहा है...",
      step2Title: "अपना पसंदीदा स्क्रिप्ट विचार चुनें!",
      tryAgainButton: "↻ नए विचारों का अनुरोध करें",
      applyIdeaButton: "✅ इस विचार का उपयोग करें!",
      cancelButton: "रद्द करें",
      contentFormats: {
        cinematic_adventure: "सिनेमैटिक साहसिक और कहानी",
        product_review: "उत्पाद समीक्षा",
        unboxing: "अनबॉक्सिंग और पहली छाप",
        vs_challenge: "तुलना वीडियो (VS चुनौती)",
      },
      characterOptions: {
        random: "यादृच्छिक रूप से चुनें",
        yourGarage: "🚗 आपका गैराज",
        construction: "🚛 निर्माण वाहन",
        beniBulldozer: "बहादुर बुलडोजर बेनी",
      },
      themeOptions: {
        random: "यादृच्छिक रूप से चुनें",
        adventureGroup: "साहसिक और अन्वेषण",
        explore_new_area: "एक नए क्षेत्र की खोज",
        rescue_mission: "दूसरे खिलौने के लिए बचाव मिशन",
        challengeGroup: "शारीरिक चुनौतियां और बाधाएं",
        overcome_obstacle: "एक विशाल बाधा पर काबू पाना",
        custom_theme: "अपना विषय टाइप करें...",
      },
      customThemePlaceholder: "जैसे, मंगल ग्रह पर रेसिंग"
    }
  },
  ar: {
    appName: "مولد الفيديو AL ANSHOR VEO",
    appTagline: "أنشئ مقاطع فيديو مذهلة من النصوص أو الصور.",
    manageStoryApiKeys: "مفتاح API للقصة",
    manageVideoApiKeys: "مفتاح API للفيديو",
    promptLabel: "موجه",
    promptPlaceholder: "أسد مهيب يطل على السافانا عند غروب الشمس...",
    promptHint: "يمكنك استخدام نص عادي أو سلسلة بتنسيق JSON.",
    referenceImageLabel: "صورة مرجعية (اختياري)",
    uploadFile: "تحميل ملف",
    dragAndDrop: "أو السحب والإفلات",
    fileTypes: "PNG, JPG, GIF حتى 10 ميجابايت",
    generationSettings: "إعدادات الإنشاء",
    aspectRatioLabel: "نسبة العرض إلى الارتفاع",
    soundLabel: "صوت",
    enableSound: "تفعيل الصوت",
    resolutionLabel: "دقة",
    videoKeyMissingWarning: "يرجى إضافة مفتاح API فيديو احترافي للبدء.",
    generateButton: "إنشاء فيديو",
    generatingButton: "جاري إنشاء الفيديو...",
    loaderTitle: "جاري إنشاء الفيديو الخاص بك",
    loadingMessages: [ "تهيئة نموذج VEO...", "تحليل موجهك...", "تكوين المشهد الرئيسي...", "إنشاء الإطارات الأولية...", "عرض تسلسل الفيديو... قد يستغرق هذا بضع دقائق.", "الترقية إلى دقة عالية...", "إضافة الصوت واللمسات النهائية...", "لقد أوشكنا على الانتهاء، جاري تحضير ملف الفيديو النهائي." ],
    playerTitle: "اكتمل الإنشاء!",
    downloadButton: "تنزيل الفيديو",
    storyApiKeyManagerTitle: "مفتاح API للقصة والبحث",
    videoApiKeyManagerTitle: "مفتاح API للفيديو الاحترافي",
    addNewStoryKeyLabel: "إضافة مفتاح API جديد للقصة",
    addNewVideoKeyLabel: "إضافة مفتاح API جديد للفيديو الاحترافي",
    apiKeyInputPlaceholder: "أدخل مفتاح Gemini API الخاص بك",
    addKeyButton: "إضافة مفتاح",
    validatingButton: "يتم التحقق...",
    savedStoryKeysLabel: "مفاتيح القصة المحفوظة",
    savedVideoKeysLabel: "مفاتيح الفيديو الاحترافية المحفوظة",
    noKeysSaved: "لا توجد مفاتيح API محفوظة.",
    addKeyPrompt: "أضف مفتاحًا أعلاه للبدء.",
    closeButton: "إغلاق",
    errorKeyEmpty: "لا يمكن أن يكون مفتاح API فارغًا.",
    errorKeyExists: "تمت إضافة مفتاح API هذا بالفعل.",
    errorKeyInvalid: "مفتاح API غير صالح أو مشكلة في الشبكة. يرجى التحقق من المفتاح والمحاولة مرة أخرى.",
    generationFailed: "فشل الإنشاء",
    alertEnterPrompt: "يرجى إدخال موجه.",
    alertSetStoryApiKey: "يرجى تعيين مفتاح API نشط للقصة قبل الإنشاء.",
    alertSetVideoApiKey: "يرجى تعيين مفتاح API نشط للفيديو الاحترافي قبل إنشاء مقطع فيديو.",
    backToStoryCreator: "العودة إلى منشئ القصة",
    storyCreator: {
      newStory: "قصة جديدة",
      characterGarage: "مرآب الشخصيات",
      garageDescription: "سجل ألعابك المفضلة هنا لجعلها النجم الرئيسي!",
      openCharacterWorkshop: "افتح ورشة الشخصيات",
      garageEmpty: "مرآب أحلامك فارغ.",
      directingDesk: "مكتب الإخراج",
      deskDescription: "إعدادات إضافية لقصتك.",
      sceneSet: "إعداد المشهد:",
      locationSet: "إعداد الموقع الرئيسي:",
      weatherSet: "إعداد الطقس:",
      cameraStyleSet: "نمط الكاميرا (POV):",
      narratorLanguageSet: "لغة الراوي:",
      customLocationPlaceholder: "مثال: ديوراما مطبخ عملاق",
      customLanguagePlaceholder: "مثال: الأردية",
      storyEditor: "محرر القصة",
      storyboard: "لوحة القصة",
      needIdea: "هل تحتاج إلى فكرة قصة؟",
      ideaDescription: "استخدم المخرج الذكي لإنشاء مخطط نصي جاهز للبث!",
      openSmartDirector: "افتح المخرج الذكي",
      storyTitle: "عنوان القصة:",
      storyTitlePlaceholder: "مثال: رينو سيارة السباق الحمراء وجورو الشاحنة الوحش الشجاعة",
      storyScript: "نص القصة / الملخص:",
      storyScriptPlaceholder: "اكتب ملخص القصة أو الفكرة الرئيسية هنا. سيقوم الذكاء الاصطناعي بتطويرها إلى مشاهد احترافية.",
      sceneCount: "عدد المشاهد:",
      createStoryboard: "أنشئ لوحة القصة!",
      storyboardPlaceholderTitle: "ستظهر نتائج قصتك هنا",
      storyboardPlaceholderDescription: "انقر على 'أنشئ لوحة القصة!' للبدء.",
      scene: "مشهد",
      cinematography: "التصوير السينمائي",
      soundEffects: "المؤثرات الصوتية",
      noSfx: "لا توجد اقتراحات للمؤثرات الصوتية.",
      mixingGuide: "دليل المزج الذكي",
      generateBlueprint: "تصميم المخطط",
      generateCinematicPrompt: "إنشاء موجه سينمائي",
      useThisPrompt: "أنشئ فيديو بهذا الموجه",
      resultBlueprint: "النتيجة: تصميم المخطط",
      resultCinematic: "النتيجة: موجه سينمائي",
      error: "خطأ",
      confirmNewStoryTitle: "هل تريد بدء قصة جديدة؟",
      confirmNewStoryMessage: "سيتم حذف كل التقدم الحالي. هل أنت متأكد؟",
      directingOptions: {
        sceneSet: { standard_cinematic: "مغامرة سينمائية قياسية", epic_destruction: "دمار ملحمي (حركة بطيئة)", drifting_precision: "تحدي الانجراف الدقيق" },
        locationSet: { standardLandGroup: "🏞️ مجموعات قياسية وأرضية", natural_outdoor: "في الهواء الطلق (حديقة / فناء)", kids_bedroom: "غرفة نوم أطفال", custom_location: "اكتب موقعك الخاص..." },
        weatherSet: { sunny: "مشمس مشرق", cloudy: "غائم", rainy: "ممطر" },
        cameraStyleSet: { standardGroup: "🎥 أنماط قياسية", standard_cinematic: "سينمائي قياسي", fpv_drone_dive: "غوص طائرة بدون طيار FPV" },
        narratorLanguageSet: { no_narrator: "بدون راوي", id: "الإندونيسية", en: "الإنجليزية", custom_language: "اكتب لغتك الخاصة..." }
      }
    },
    characterWorkshop: {
      titleCreate: "ورشة عمل شخصيات الذكاء الاصطناعي",
      titleEdit: "تحرير الشخصية",
      step1Title: "الخطوة 1 من 3: قم بتحميل صورة أو صف فكرة لعبتك.",
      imagePreview: "معاينة الصورة",
      uploadButton: "تحميل صورة مرجعية",
      ideaPlaceholder: "أضف ملاحظات أو وصفًا إضافيًا (اختياري إذا تم تحميل الصورة)...",
      designButton: "تصميم نموذج فريد",
      designingButton: "جاري التصميم...",
      step2Title: "الخطوة 2 من 3: راجع وحرر تفاصيل النموذج التي أنشأها الذكاء الاصطناعي.",
      brandName: "اسم علامة تجارية وهمية:",
      modelName: "اسم نموذج محدد:",
      consistencyId: "معرف الاتساق (رمز فريد):",
      consistencyIdHint: "استخدم هذا المعرف الفريد في كل موجه للحفاظ على اتساق الشخصية.",
      mainMaterial: "المادة الرئيسية:",
      designLanguage: "لغة تصميم العلامة التجارية:",
      keyFeatures: "الميزات الرئيسية (الحمض النووي البصري):",
      step3Title: "الخطوة 3 من 3: حدد \"الحمض النووي للحركة\" لهذه اللعبة.",
      actionDnaSuggestions: "اقتراحات الحمض النووي للحركة من الذكاء الاصطناعي:",
      addCustomAction: "إضافة حركة مخصصة:",
      customActionPlaceholder: "مثال: الانزلاق على الماء",
      addButton: "إضافة",
      selectedActionDna: "الحمض النووي للحركة المحدد:",
      nextButton: "التالي",
      backButton: "رجوع",
      saveButton: "حفظ الشخصية",
      updateButton: "تحديث الشخصية",
      alertUploadOrDescribe: "يرجى تحميل صورة أو وصف فكرة لعبتك.",
      alertRequiredFields: "اسم العلامة التجارية واسم الطراز ومعرف الاتساق مطلوبة."
    },
    smartDirector: {
      title: "المخرج الذكي",
      step1Description: "دعنا ننشئ مخطط نصي جاهز للبث! اتبع هذه الخطوات السهلة.",
      step1Label: "الخطوة 1: اختر تنسيق المحتوى",
      step2Label: "الخطوة 2: اختر الشخصية الرئيسية",
      step3Label: "الخطوة 3: اختر موضوع القصة",
      generateIdeasButton: "أعطني 3 أفكار!",
      generatingIdeasButton: "يفكر...",
      step2Title: "اختر فكرة النص المفضلة لديك!",
      tryAgainButton: "↻ طلب أفكار جديدة",
      applyIdeaButton: "✅ استخدم هذه الفكرة!",
      cancelButton: "إلغاء",
      contentFormats: {
        cinematic_adventure: "مغامرة وقصة سينمائية",
        product_review: "مراجعة المنتج",
        unboxing: "فتح العلبة والانطباعات الأولى",
        vs_challenge: "فيديو مقارنة (تحدي VS)",
      },
      characterOptions: {
        random: "اختر بشكل عشوائي",
        yourGarage: "🚗 المرآب الخاص بك",
        construction: "🚛 مركبات البناء",
        beniBulldozer: "بيني الجرافة الشجاعة",
      },
      themeOptions: {
        random: "اختر بشكل عشوائي",
        adventureGroup: "مغامرة واستكشاف",
        explore_new_area: "استكشاف منطقة جديدة",
        rescue_mission: "مهمة إنقاذ لعبة أخرى",
        challengeGroup: "تحديات وعقبات جسدية",
        overcome_obstacle: "التغلب على عقبة عملاقة",
        custom_theme: "اكتب موضوعك الخاص...",
      },
      customThemePlaceholder: "مثال: سباق على كوكب المريخ"
    }
  },
  pt: {
    appName: "GERADOR DE VÍDEO AL ANSHOR VEO",
    appTagline: "Gere vídeos impressionantes a partir de texto ou imagens.",
    manageStoryApiKeys: "Chave de API da História",
    manageVideoApiKeys: "Chave de API de Vídeo",
    promptLabel: "Comando",
    promptPlaceholder: "Um leão majestoso observando a savana ao pôr do sol...",
    promptHint: "Você pode usar texto simples ou uma string formatada em JSON.",
    referenceImageLabel: "Imagem de Referência (Opcional)",
    uploadFile: "Carregar um arquivo",
    dragAndDrop: "ou arraste e solte",
    fileTypes: "PNG, JPG, GIF até 10MB",
    generationSettings: "Configurações de Geração",
    aspectRatioLabel: "Proporção da Tela",
    soundLabel: "Som",
    enableSound: "Habilitar som",
    resolutionLabel: "Resolução",
    videoKeyMissingWarning: "Adicione uma Chave de API de Vídeo profissional para começar.",
    generateButton: "Gerar Vídeo",
    generatingButton: "Gerando Vídeo...",
    loaderTitle: "Gerando Seu Vídeo",
    loadingMessages: [ "Inicializando modelo VEO...", "Analisando seu comando...", "Compondo a cena principal...", "Gerando quadros iniciais...", "Renderizando sequência de vídeo... isso pode levar alguns minutos.", "Ampliando para alta resolução...", "Adicionando som e toques finais...", "Quase lá, preparando o arquivo de vídeo final." ],
    playerTitle: "Geração Concluída!",
    downloadButton: "Baixar Vídeo",
    storyApiKeyManagerTitle: "Chave de API de História e Pesquisa",
    videoApiKeyManagerTitle: "Chave de API de Vídeo Profissional",
    addNewStoryKeyLabel: "Adicionar Nova Chave de API de História",
    addNewVideoKeyLabel: "Adicionar Nova Chave de API de Vídeo Profissional",
    apiKeyInputPlaceholder: "Digite sua Chave de API do Gemini",
    addKeyButton: "Adicionar Chave",
    validatingButton: "Validando...",
    savedStoryKeysLabel: "Chaves de História Salvas",
    savedVideoKeysLabel: "Chaves de Vídeo Profissional Salvas",
    noKeysSaved: "Nenhuma Chave de API salva.",
    addKeyPrompt: "Adicione uma chave acima para começar.",
    closeButton: "Fechar",
    errorKeyEmpty: "A Chave de API não pode estar vazia.",
    errorKeyExists: "Esta Chave de API já foi adicionada.",
    errorKeyInvalid: "Chave de API inválida ou problema de rede. Verifique a chave e tente novamente.",
    generationFailed: "Falha na Geração",
    alertEnterPrompt: "Por favor, insira um comando.",
    alertSetStoryApiKey: "Defina uma chave de API de História ativa antes de gerar.",
    alertSetVideoApiKey: "Por favor, defina uma chave de API de Vídeo Profissional ativa antes de gerar um vídeo.",
    backToStoryCreator: "Voltar para o Criador de Histórias",
    storyCreator: {
      newStory: "Nova História",
      characterGarage: "Garagem de Personagens",
      garageDescription: "Cadastre seus brinquedos favoritos aqui para torná-los a estrela principal!",
      openCharacterWorkshop: "Abrir Oficina de Personagens",
      garageEmpty: "Sua garagem dos sonhos está vazia.",
      directingDesk: "Mesa de Direção",
      deskDescription: "Configurações adicionais para sua história.",
      sceneSet: "Conjunto de Cena:",
      locationSet: "Conjunto de Local Principal:",
      weatherSet: "Conjunto de Clima:",
      cameraStyleSet: "Estilo de Câmera (POV):",
      narratorLanguageSet: "Idioma do Narrador:",
      customLocationPlaceholder: "Ex: Diorama de Cozinha Gigante",
      customLanguagePlaceholder: "Ex: Galego",
      storyEditor: "Editor de Histórias",
      storyboard: "Storyboard",
      needIdea: "Precisa de uma ideia para a história?",
      ideaDescription: "Use o Diretor Inteligente para criar um esboço de roteiro pronto para o ar!",
      openSmartDirector: "Abrir Diretor Inteligente",
      storyTitle: "Título da História:",
      storyTitlePlaceholder: "Ex: Rino, o Carro de Corrida Vermelho e Goro, o Caminhão Monstro Corajoso",
      storyScript: "Roteiro / Resumo da História:",
      storyScriptPlaceholder: "Escreva um resumo da história ou a ideia principal aqui. A IA o desenvolverá em cenas profissionais.",
      sceneCount: "Número de Cenas:",
      createStoryboard: "Criar Storyboard!",
      storyboardPlaceholderTitle: "Os Resultados da Sua História Aparecerão Aqui",
      storyboardPlaceholderDescription: "Clique em 'Criar Storyboard!' para começar.",
      scene: "Cena",
      cinematography: "Cinematografia",
      soundEffects: "Efeitos Sonoros",
      noSfx: "Nenhuma sugestão de SFX.",
      mixingGuide: "Guia de Mixagem Inteligente",
      generateBlueprint: "Projetar Blueprint",
      generateCinematicPrompt: "Criar Prompt Cinemático",
      useThisPrompt: "Gerar Vídeo com este Prompt",
      resultBlueprint: "Resultado: Design do Blueprint",
      resultCinematic: "Resultado: Prompt Cinemático",
      error: "Erro",
      confirmNewStoryTitle: "Começar uma Nova História?",
      confirmNewStoryMessage: "Todo o progresso atual será excluído. Você tem certeza?",
      directingOptions: {
        sceneSet: { standard_cinematic: "Aventura Cinemática Padrão", epic_destruction: "Destruição Épica (Câmera Lenta)", drifting_precision: "Desafio de Precisão de Drift" },
        locationSet: { standardLandGroup: "🏞️ Conjuntos Padrão e Terrestres", natural_outdoor: "Ao Ar Livre (Jardim/Quintal)", kids_bedroom: "Quarto de Criança", custom_location: "Digite seu próprio local..." },
        weatherSet: { sunny: "Ensolarado", cloudy: "Nublado", rainy: "Chuvoso" },
        cameraStyleSet: { standardGroup: "🎥 Estilos Padrão", standard_cinematic: "Cinemático Padrão", fpv_drone_dive: "Mergulho de Drone FPV" },
        narratorLanguageSet: { no_narrator: "Sem Narrador", id: "Indonésio", en: "Inglês", custom_language: "Digite seu próprio idioma..." }
      }
    },
    characterWorkshop: {
      titleCreate: "Oficina de Personagens com IA",
      titleEdit: "Editar Personagem",
      step1Title: "Passo 1 de 3: Carregue uma imagem ou descreva sua ideia de brinquedo.",
      imagePreview: "Pré-visualização da Imagem",
      uploadButton: "Carregar Imagem de Referência",
      ideaPlaceholder: "Adicione notas ou descrição adicional (opcional se a imagem for carregada)...",
      designButton: "Projetar Modelo Único",
      designingButton: "Projetando...",
      step2Title: "Passo 2 de 3: Revise e edite os detalhes do modelo gerados pela IA.",
      brandName: "Nome da Marca Fictícia:",
      modelName: "Nome do Modelo Específico:",
      consistencyId: "ID de Consistência (Token Único):",
      consistencyIdHint: "Use este ID único em cada comando para manter a consistência do personagem.",
      mainMaterial: "Material Principal:",
      designLanguage: "Linguagem de Design da Marca:",
      keyFeatures: "Características Principais (DNA Visual):",
      step3Title: "Passo 3 de 3: Defina o \"DNA de Ação\" deste brinquedo.",
      actionDnaSuggestions: "Sugestões de DNA de Ação da IA:",
      addCustomAction: "Adicionar Ação Personalizada:",
      customActionPlaceholder: "Ex: Deslizar na Água",
      addButton: "Adicionar",
      selectedActionDna: "DNA de Ação Selecionado:",
      nextButton: "Próximo",
      backButton: "Voltar",
      saveButton: "Salvar Personagem",
      updateButton: "Atualizar Personagem",
      alertUploadOrDescribe: "Por favor, carregue uma imagem ou descreva sua ideia de brinquedo.",
      alertRequiredFields: "Nome da Marca, Nome do Modelo e ID de Consistência são obrigatórios."
    },
    smartDirector: {
      title: "Diretor Inteligente",
      step1Description: "Vamos criar um esboço de roteiro pronto para o ar! Siga estes passos fáceis.",
      step1Label: "Passo 1: Escolha o Formato do Conteúdo",
      step2Label: "Passo 2: Escolha o Personagem Principal",
      step3Label: "Passo 3: Escolha o Tema da História",
      generateIdeasButton: "Dê-me 3 Ideias!",
      generatingIdeasButton: "Pensando...",
      step2Title: "Escolha sua Ideia de Roteiro Favorita!",
      tryAgainButton: "↻ Solicitar Novas Ideias",
      applyIdeaButton: "✅ Usar esta Ideia!",
      cancelButton: "Cancelar",
      contentFormats: {
        cinematic_adventure: "Aventura e História Cinemática",
        product_review: "Análise de Produto",
        unboxing: "Unboxing e Primeiras Impressões",
        vs_challenge: "Vídeo de Comparação (Desafio VS)",
      },
      characterOptions: {
        random: "Escolher Aleatoriamente",
        yourGarage: "🚗 Sua Garagem",
        construction: "🚛 Veículos de Construção",
        beniBulldozer: "Beni, o Bulldozer Corajoso",
      },
      themeOptions: {
        random: "Escolher Aleatoriamente",
        adventureGroup: "Aventura e Exploração",
        explore_new_area: "Explorando uma Nova Área",
        rescue_mission: "Missão de Resgate para Outro Brinquedo",
        challengeGroup: "Desafios e Obstáculos Físicos",
        overcome_obstacle: "Superando um Obstáculo Gigante",
        custom_theme: "Digite seu próprio tema...",
      },
      customThemePlaceholder: "Ex: Corrida no Planeta Marte"
    }
  },
  bn: {
    appName: "AL ANSHOR VEO জেনারেটর",
    appTagline: "টেক্সট বা ছবি থেকে потрясающие ভিডিও তৈরি করুন।",
    manageStoryApiKeys: "গল্পের API কী",
    manageVideoApiKeys: "ভিডিও API কী",
    promptLabel: "প্রম্পট",
    promptPlaceholder: "সূর্যাস্তের সময় সাভানার দিকে তাকিয়ে থাকা এক মহিমান্বিত সিংহ...",
    promptHint: "আপনি সাধারণ টেক্সট বা JSON ফরম্যাটেড স্ট্রিং ব্যবহার করতে পারেন।",
    referenceImageLabel: "রেফারেন্স চিত্র (ঐচ্ছিক)",
    uploadFile: "ফাইল আপলোড করুন",
    dragAndDrop: "অথবা টেনে আনুন",
    fileTypes: "PNG, JPG, GIF 10MB পর্যন্ত",
    generationSettings: "জেনারেটর সেটিংস",
    aspectRatioLabel: "আনুমানিক অনুপাত",
    soundLabel: "শব্দ",
    enableSound: "শব্দ সক্রিয় করুন",
    resolutionLabel: "রেজোলিউশন",
    videoKeyMissingWarning: "শুরু করার জন্য অনুগ্রহ করে একটি পেশাদার ভিডিও API কী যোগ করুন।",
    generateButton: "ভিডিও তৈরি করুন",
    generatingButton: "ভিডিও তৈরি হচ্ছে...",
    loaderTitle: "আপনার ভিডিও তৈরি হচ্ছে",
    loadingMessages: [ "VEO মডেল শুরু হচ্ছে...", "আপনার প্রম্পট বিশ্লেষণ করা হচ্ছে...", "মূল দৃশ্য রচনা করা হচ্ছে...", "প্রাথমিক ফ্রেম তৈরি হচ্ছে...", "ভিডিও ক্রম রেন্ডার করা হচ্ছে... এতে কয়েক মিনিট সময় লাগতে পারে।", "উচ্চ রেজোলিউশনে আপস্কেলিং...", "শব্দ এবং চূড়ান্ত ছোঁয়া যোগ করা হচ্ছে...", "প্রায় শেষ, চূড়ান্ত ভিডিও ফাইল প্রস্তুত করা হচ্ছে।" ],
    playerTitle: "তৈরি সম্পন্ন!",
    downloadButton: "ভিডিও ডাউনলোড করুন",
    storyApiKeyManagerTitle: "গল্প ও গবেষণা API কী",
    videoApiKeyManagerTitle: "পেশাদার ভিডিও API কী",
    addNewStoryKeyLabel: "নতুন গল্পের API কী যোগ করুন",
    addNewVideoKeyLabel: "নতুন পেশাদার ভিডিও API কী যোগ করুন",
    apiKeyInputPlaceholder: "আপনার জেমিনি API কী লিখুন",
    addKeyButton: "কী যোগ করুন",
    validatingButton: "যাচাই করা হচ্ছে...",
    savedStoryKeysLabel: "সংরক্ষিত গল্পের কী",
    savedVideoKeysLabel: "সংরক্ষিত পেশাদার ভিডিও কী",
    noKeysSaved: "কোনো API কী সংরক্ষিত নেই।",
    addKeyPrompt: "শুরু করার জন্য উপরে একটি কী যোগ করুন।",
    closeButton: "বন্ধ করুন",
    errorKeyEmpty: "API কী খালি হতে পারে না।",
    errorKeyExists: "এই API কী ইতিমধ্যে যোগ করা হয়েছে।",
    errorKeyInvalid: "অবৈধ API কী বা নেটওয়ার্ক সমস্যা। অনুগ্রহ করে কী পরীক্ষা করে আবার চেষ্টা করুন।",
    generationFailed: "তৈরি ব্যর্থ হয়েছে",
    alertEnterPrompt: "অনুগ্রহ করে একটি প্রম্পট লিখুন।",
    alertSetStoryApiKey: "তৈরি করার আগে অনুগ্রহ করে একটি সক্রিয় গল্পের API কী সেট করুন।",
    alertSetVideoApiKey: "ভিডিও তৈরি করার আগে অনুগ্রহ করে একটি সক্রিয় পেশাদার ভিডিও API কী সেট করুন।",
    backToStoryCreator: "গল্প নির্মাতার কাছে ফিরে যান",
    storyCreator: {
      newStory: "নতুন গল্প",
      characterGarage: "চরিত্রের গ্যারেজ",
      garageDescription: "আপনার প্রিয় খেলনাগুলো এখানে নিবন্ধন করুন যাতে তারা প্রধান তারকা হতে পারে!",
      openCharacterWorkshop: "চরিত্র কর্মশালা খুলুন",
      garageEmpty: "আপনার স্বপ্নের গ্যারেজ খালি।",
      directingDesk: "পরিচালনার ডেস্ক",
      deskDescription: "আপনার গল্পের জন্য অতিরিক্ত সেটিংস।",
      sceneSet: "দৃশ্যের সেট:",
      locationSet: "প্রধান অবস্থানের সেট:",
      weatherSet: "আবহাওয়ার সেট:",
      cameraStyleSet: "ক্যামেরার স্টাইল (POV):",
      narratorLanguageSet: "বর্ণনাকারীর ভাষার সেট:",
      customLocationPlaceholder: "যেমন, বিশাল রান্নাঘরের ডায়োরামা",
      customLanguagePlaceholder: "যেমন, অসমীয়া",
      storyEditor: "গল্প সম্পাদক",
      storyboard: "স্টোরিবোর্ড",
      needIdea: "গল্পের ধারণা প্রয়োজন?",
      ideaDescription: "সম্প্রচারের জন্য প্রস্তুত একটি স্ক্রিপ্ট আউটলাইন তৈরি করতে স্মার্ট ডিরেক্টর ব্যবহার করুন!",
      openSmartDirector: "স্মার্ট ডিরেক্টর খুলুন",
      storyTitle: "গল্পের শিরোনাম:",
      storyTitlePlaceholder: "যেমন, লাল রেসিং কার রিনো এবং সাহসী মনস্টার ট্রাক গোরো",
      storyScript: "গল্পের স্ক্রিপ্ট / সারসংক্ষেপ:",
      storyScriptPlaceholder: "এখানে একটি গল্পের সারসংক্ষেপ বা মূল ধারণা লিখুন। AI এটিকে পেশাদার দৃশ্যে রূপান্তরিত করবে।",
      sceneCount: "দৃশ্যের সংখ্যা:",
      createStoryboard: "স্টোরিবোর্ড তৈরি করুন!",
      storyboardPlaceholderTitle: "আপনার গল্পের ফলাফল এখানে প্রদর্শিত হবে",
      storyboardPlaceholderDescription: "শুরু করতে 'স্টোরিবোর্ড তৈরি করুন!' ক্লিক করুন।",
      scene: "দৃশ্য",
      cinematography: "সিনেমাটোগ্রাফি",
      soundEffects: "শব্দ প্রভাব",
      noSfx: "কোনো SFX পরামর্শ নেই।",
      mixingGuide: "স্মার্ট মিক্সিং গাইড",
      generateBlueprint: "ব্লুপ্রিন্ট ডিজাইন করুন",
      generateCinematicPrompt: "সিনেমাটিক প্রম্পট তৈরি করুন",
      useThisPrompt: "এই প্রম্পট দিয়ে ভিডিও তৈরি করুন",
      resultBlueprint: "ফলাফল: ব্লুপ্রিন্ট ডিজাইন",
      resultCinematic: "ফলাফল: সিনেমাটিক প্রম্পট",
      error: "ত্রুটি",
      confirmNewStoryTitle: "একটি নতুন গল্প শুরু করবেন?",
      confirmNewStoryMessage: "সমস্ত বর্তমান অগ্রগতি মুছে ফেলা হবে। আপনি কি নিশ্চিত?",
      directingOptions: {
        sceneSet: { standard_cinematic: "স্ট্যান্ডার্ড সিনেমাটিক অ্যাডভেঞ্চার", epic_destruction: "এপিক ধ্বংস (স্লো-মোশন)", drifting_precision: "ড্রিফটিং প্রিসিশন চ্যালেঞ্জ" },
        locationSet: { standardLandGroup: "🏞️ স্ট্যান্ডার্ড ও ল্যান্ড সেট", natural_outdoor: "বাইরে (বাগান/উঠান)", kids_bedroom: "বাচ্চার শোবার ঘর", custom_location: "আপনার নিজের অবস্থান টাইপ করুন..." },
        weatherSet: { sunny: "উজ্জ্বল রোদ", cloudy: "মেঘলা", rainy: "বৃষ্টি" },
        cameraStyleSet: { standardGroup: "🎥 স্ট্যান্ডার্ড স্টাইল", standard_cinematic: "স্ট্যান্ডার্ড সিনেমাটিক", fpv_drone_dive: "FPV ড্রোন ডাইভ" },
        narratorLanguageSet: { no_narrator: "বর্ণনাকারী ছাড়া", id: "ইন্দোনেশিয়ান", en: "ইংরেজি", custom_language: "আপনার নিজের ভাষা টাইপ করুন..." }
      }
    },
    characterWorkshop: {
      titleCreate: "AI চরিত্র কর্মশালা",
      titleEdit: "চরিত্র সম্পাদনা করুন",
      step1Title: "পদক্ষেপ ১/৩: একটি ছবি আপলোড করুন বা আপনার খেলনার ধারণা বর্ণনা করুন।",
      imagePreview: "ছবির পূর্বরূপ",
      uploadButton: "রেফারেন্স ছবি আপলোড করুন",
      ideaPlaceholder: "নোট বা অতিরিক্ত বর্ণনা যোগ করুন (ছবি আপলোড করা হলে ঐচ্ছিক)...",
      designButton: "অনন্য মডেল ডিজাইন করুন",
      designingButton: "ডিজাইন করা হচ্ছে...",
      step2Title: "পদক্ষেপ ২/৩: AI-সৃষ্ট মডেলের বিবরণ পর্যালোচনা এবং সম্পাদনা করুন।",
      brandName: "কাল্পনিক ব্র্যান্ডের নাম:",
      modelName: "নির্দিষ্ট মডেলের নাম:",
      consistencyId: "ধারাবাহিকতা আইডি (অনন্য টোকেন):",
      consistencyIdHint: "চরিত্রের ধারাবাহিকতা বজায় রাখতে প্রতিটি প্রম্পটে এই অনন্য আইডি ব্যবহার করুন।",
      mainMaterial: "প্রধান উপাদান:",
      designLanguage: "ব্র্যান্ড ডিজাইন ভাষা:",
      keyFeatures: "মূল বৈশিষ্ট্য (ভিজ্যুয়াল ডিএনএ):",
      step3Title: "পদক্ষেপ ৩/৩: এই খেলনার \"অ্যাকশন ডিএনএ\" নির্ধারণ করুন।",
      actionDnaSuggestions: "AI অ্যাকশন ডিএনএ পরামর্শ:",
      addCustomAction: "কাস্টম অ্যাকশন যোগ করুন:",
      customActionPlaceholder: "যেমন, জলে স্কিমিং",
      addButton: "যোগ করুন",
      selectedActionDna: "নির্বাচিত অ্যাকশন ডিএনএ:",
      nextButton: "পরবর্তী",
      backButton: "ফিরে যান",
      saveButton: "চরিত্র সংরক্ষণ করুন",
      updateButton: "চরিত্র আপডেট করুন",
      alertUploadOrDescribe: "অনুগ্রহ করে একটি ছবি আপলোড করুন বা আপনার খেলনার ধারণা বর্ণনা করুন।",
      alertRequiredFields: "ব্র্যান্ডের নাম, মডেলের নাম এবং ধারাবাহিকতা আইডি আবশ্যক।"
    },
    smartDirector: {
      title: "স্মার্ট ডিরেক্টর",
      step1Description: "চলুন একটি সম্প্রচারের জন্য প্রস্তুত স্ক্রিপ্ট আউটলাইন তৈরি করি! এই সহজ পদক্ষেপগুলো অনুসরণ করুন।",
      step1Label: "পদক্ষেপ ১: কন্টেন্ট ফরম্যাট বেছে নিন",
      step2Label: "পদক্ষেপ ২: প্রধান চরিত্র বেছে নিন",
      step3Label: "পদক্ষেপ ৩: গল্পের থিম বেছে নিন",
      generateIdeasButton: "আমাকে ৩টি ধারণা দিন!",
      generatingIdeasButton: "ভাবছে...",
      step2Title: "আপনার প্রিয় স্ক্রিপ্ট ধারণাটি বেছে নিন!",
      tryAgainButton: "↻ নতুন ধারণার অনুরোধ করুন",
      applyIdeaButton: "✅ এই ধারণাটি ব্যবহার করুন!",
      cancelButton: "বাতিল করুন",
      contentFormats: {
        cinematic_adventure: "সিনেমাটিক অ্যাডভেঞ্চার ও গল্প",
        product_review: "পণ্য পর্যালোচনা",
        unboxing: "আনবক্সিং ও প্রথম ইমপ্রেশন",
        vs_challenge: "তুলনামূলক ভিডিও (VS চ্যালেঞ্জ)",
      },
      characterOptions: {
        random: "এলোমেলোভাবে বেছে নিন",
        yourGarage: "🚗 আপনার গ্যারেজ",
        construction: "🚛 নির্মাণ যানবাহন",
        beniBulldozer: "সাহসী বুলডোজার বেনি",
      },
      themeOptions: {
        random: "এলোমেলোভাবে বেছে নিন",
        adventureGroup: "অ্যাডভেঞ্চার ও অন্বেষণ",
        explore_new_area: "একটি নতুন এলাকা অন্বেষণ",
        rescue_mission: "অন্য খেলনার জন্য উদ্ধার মিশন",
        challengeGroup: "শারীরিক চ্যালেঞ্জ ও বাধা",
        overcome_obstacle: "একটি বিশাল বাধা অতিক্রম করা",
        custom_theme: "আপনার নিজের থিম টাইপ করুন...",
      },
      customThemePlaceholder: "যেমন, মঙ্গল গ্রহে রেসিং"
    }
  },
  ru: {
    appName: "ГЕНЕРАТОР ВИДЕО AL ANSHOR VEO",
    appTagline: "Создавайте потрясающие видео из текста или изображений.",
    manageStoryApiKeys: "Ключ API для историй",
    manageVideoApiKeys: "Ключ API для видео",
    promptLabel: "Запрос",
    promptPlaceholder: "Величественный лев, взирающий на саванну на закате...",
    promptHint: "Вы можете использовать обычный текст или строку в формате JSON.",
    referenceImageLabel: "Эталонное изображение (необязательно)",
    uploadFile: "Загрузить файл",
    dragAndDrop: "или перетащить",
    fileTypes: "PNG, JPG, GIF до 10 МБ",
    generationSettings: "Настройки генерации",
    aspectRatioLabel: "Соотношение сторон",
    soundLabel: "Звук",
    enableSound: "Включить звук",
    resolutionLabel: "Разрешение",
    videoKeyMissingWarning: "Пожалуйста, добавьте профессиональный ключ API для видео, чтобы начать.",
    generateButton: "Создать видео",
    generatingButton: "Создание видео...",
    loaderTitle: "Создание вашего видео",
    loadingMessages: [ "Инициализация модели VEO...", "Анализ вашего запроса...", "Компоновка основной сцены...", "Генерация начальных кадров...", "Рендеринг видеопоследовательности... это может занять несколько минут.", "Масштабирование до высокого разрешения...", "Добавление звука и финальные штрихи...", "Почти готово, подготовка финального видеофайла." ],
    playerTitle: "Создание завершено!",
    downloadButton: "Скачать видео",
    storyApiKeyManagerTitle: "Ключ API для историй и исследований",
    videoApiKeyManagerTitle: "Профессиональный ключ API для видео",
    addNewStoryKeyLabel: "Добавить новый ключ API для историй",
    addNewVideoKeyLabel: "Добавить новый профессиональный ключ API для видео",
    apiKeyInputPlaceholder: "Введите ваш ключ API Gemini",
    addKeyButton: "Добавить ключ",
    validatingButton: "Проверка...",
    savedStoryKeysLabel: "Сохраненные ключи для историй",
    savedVideoKeysLabel: "Сохраненные профессиональные ключи для видео",
    noKeysSaved: "Нет сохраненных ключей API.",
    addKeyPrompt: "Добавьте ключ выше, чтобы начать.",
    closeButton: "Закрыть",
    errorKeyEmpty: "Ключ API не может быть пустым.",
    errorKeyExists: "Этот ключ API уже добавлен.",
    errorKeyInvalid: "Неверный ключ API или проблема с сетью. Пожалуйста, проверьте ключ и попробуйте снова.",
    generationFailed: "Ошибка создания",
    alertEnterPrompt: "Пожалуйста, введите запрос.",
    alertSetStoryApiKey: "Пожалуйста, установите активный ключ API для историй перед созданием.",
    alertSetVideoApiKey: "Пожалуйста, установите активный профессиональный ключ API для видео перед созданием видео.",
    backToStoryCreator: "Вернуться в Создатель историй",
    storyCreator: {
      newStory: "Новая история",
      characterGarage: "Гараж персонажей",
      garageDescription: "Зарегистрируйте здесь свои любимые игрушки, чтобы сделать их главной звездой!",
      openCharacterWorkshop: "Открыть мастерскую персонажей",
      garageEmpty: "Гараж вашей мечты пуст.",
      directingDesk: "Режиссерский стол",
      deskDescription: "Дополнительные настройки для вашей истории.",
      sceneSet: "Набор сцены:",
      locationSet: "Основное местоположение:",
      weatherSet: "Погода:",
      cameraStyleSet: "Стиль камеры (POV):",
      narratorLanguageSet: "Язык рассказчика:",
      customLocationPlaceholder: "Например, диорама гигантской кухни",
      customLanguagePlaceholder: "Например, украинский",
      storyEditor: "Редактор историй",
      storyboard: "Раскадровка",
      needIdea: "Нужна идея для истории?",
      ideaDescription: "Используйте Умного режиссера, чтобы создать готовый к эфиру набросок сценария!",
      openSmartDirector: "Открыть Умного режиссера",
      storyTitle: "Название истории:",
      storyTitlePlaceholder: "Например, Рино - красная гоночная машина и Горо - храбрый монстр-трак",
      storyScript: "Сценарий / Краткое содержание:",
      storyScriptPlaceholder: "Напишите здесь краткое содержание истории или основную идею. ИИ разовьет ее в профессиональные сцены.",
      sceneCount: "Количество сцен:",
      createStoryboard: "Создать раскадровку!",
      storyboardPlaceholderTitle: "Результаты вашей истории появятся здесь",
      storyboardPlaceholderDescription: "Нажмите 'Создать раскадровку!', чтобы начать.",
      scene: "Сцена",
      cinematography: "Кинематография",
      soundEffects: "Звуковые эффекты",
      noSfx: "Нет предложений по SFX.",
      mixingGuide: "Умное руководство по сведению",
      generateBlueprint: "Разработать чертеж",
      generateCinematicPrompt: "Создать кинематографический запрос",
      useThisPrompt: "Создать видео с этим запросом",
      resultBlueprint: "Результат: Дизайн чертежа",
      resultCinematic: "Результат: Кинематографический запрос",
      error: "Ошибка",
      confirmNewStoryTitle: "Начать новую историю?",
      confirmNewStoryMessage: "Весь текущий прогресс будет удален. Вы уверены?",
      directingOptions: {
        sceneSet: { standard_cinematic: "Стандартное кинематографическое приключение", epic_destruction: "Эпическое разрушение (замедленная съемка)", drifting_precision: "Испытание на точность дрифта" },
        locationSet: { standardLandGroup: "🏞️ Стандартные и наземные наборы", natural_outdoor: "На открытом воздухе (сад/двор)", kids_bedroom: "Детская спальня", custom_location: "Введите свое местоположение..." },
        weatherSet: { sunny: "Ясно", cloudy: "Облачно", rainy: "Дождь" },
        cameraStyleSet: { standardGroup: "🎥 Стандартные стили", standard_cinematic: "Стандартный кинематографический", fpv_drone_dive: "Погружение с FPV-дрона" },
        narratorLanguageSet: { no_narrator: "Без рассказчика", id: "Индонезийский", en: "Английский", custom_language: "Введите свой язык..." }
      }
    },
    characterWorkshop: {
      titleCreate: "Мастерская персонажей ИИ",
      titleEdit: "Редактировать персонажа",
      step1Title: "Шаг 1 из 3: Загрузите изображение или опишите свою идею игрушки.",
      imagePreview: "Предпросмотр изображения",
      uploadButton: "Загрузить эталонное изображение",
      ideaPlaceholder: "Добавьте заметки или дополнительное описание (необязательно, если изображение загружено)...",
      designButton: "Разработать уникальную модель",
      designingButton: "Разработка...",
      step2Title: "Шаг 2 из 3: Просмотрите и отредактируйте детали модели, созданные ИИ.",
      brandName: "Вымышленное название бренда:",
      modelName: "Конкретное название модели:",
      consistencyId: "ID согласованности (уникальный токен):",
      consistencyIdHint: "Используйте этот уникальный ID в каждом запросе для поддержания согласованности персонажа.",
      mainMaterial: "Основной материал:",
      designLanguage: "Язык дизайна бренда:",
      keyFeatures: "Ключевые особенности (визуальная ДНК):",
      step3Title: "Шаг 3 из 3: Определите «ДНК действия» этой игрушки.",
      actionDnaSuggestions: "Предложения ДНК действия от ИИ:",
      addCustomAction: "Добавить пользовательское действие:",
      customActionPlaceholder: "Например, скольжение по воде",
      addButton: "Добавить",
      selectedActionDna: "Выбранная ДНК действия:",
      nextButton: "Далее",
      backButton: "Назад",
      saveButton: "Сохранить персонажа",
      updateButton: "Обновить персонажа",
      alertUploadOrDescribe: "Пожалуйста, загрузите изображение или опишите свою идею игрушки.",
      alertRequiredFields: "Название бренда, название модели и ID согласованности обязательны для заполнения."
    },
    smartDirector: {
      title: "Умный режиссер",
      step1Description: "Давайте создадим набросок сценария, готовый к эфиру! Следуйте этим простым шагам.",
      step1Label: "Шаг 1: Выберите формат контента",
      step2Label: "Шаг 2: Выберите главного героя",
      step3Label: "Шаг 3: Выберите тему истории",
      generateIdeasButton: "Дайте 3 идеи!",
      generatingIdeasButton: "Думаю...",
      step2Title: "Выберите свою любимую идею сценария!",
      tryAgainButton: "↻ Запросить новые идеи",
      applyIdeaButton: "✅ Использовать эту идею!",
      cancelButton: "Отмена",
      contentFormats: {
        cinematic_adventure: "Кинематографическое приключение и история",
        product_review: "Обзор продукта",
        unboxing: "Распаковка и первые впечатления",
        vs_challenge: "Сравнительное видео (VS-челлендж)",
      },
      characterOptions: {
        random: "Выбрать случайно",
        yourGarage: "🚗 Ваш гараж",
        construction: "🚛 Строительная техника",
        beniBulldozer: "Храбрый бульдозер Бени",
      },
      themeOptions: {
        random: "Выбрать случайно",
        adventureGroup: "Приключения и исследования",
        explore_new_area: "Исследование новой области",
        rescue_mission: "Миссия по спасению другой игрушки",
        challengeGroup: "Физические испытания и препятствия",
        overcome_obstacle: "Преодоление гигантского препятствия",
        custom_theme: "Введите свою тему...",
      },
      customThemePlaceholder: "Например, гонки на Марсе"
    }
  },
  ja: {
    appName: "AL ANSHOR VEO ビデオジェネレーター",
    appTagline: "テキストや画像から素晴らしいビデオを生成します。",
    manageStoryApiKeys: "ストーリーAPIキー",
    manageVideoApiKeys: "ビデオAPIキー",
    promptLabel: "プロンプト",
    promptPlaceholder: "夕日に照らされたサバンナを見下ろす雄大なライオン...",
    promptHint: "プレーンテキストまたはJSON形式の文字列を使用できます。",
    referenceImageLabel: "参照画像（オプション）",
    uploadFile: "ファイルをアップロード",
    dragAndDrop: "またはドラッグ＆ドロップ",
    fileTypes: "PNG, JPG, GIF 最大10MB",
    generationSettings: "生成設定",
    aspectRatioLabel: "アスペクト比",
    soundLabel: "サウンド",
    enableSound: "サウンドを有効にする",
    resolutionLabel: "解像度",
    videoKeyMissingWarning: "開始するには、プロフェッショナルビデオAPIキーを追加してください。",
    generateButton: "ビデオを生成",
    generatingButton: "ビデオを生成中...",
    loaderTitle: "ビデオを生成中",
    loadingMessages: [ "VEOモデルを初期化中...", "プロンプトを分析中...", "メインシーンを作成中...", "初期フレームを生成中...", "ビデオシーケンスをレンダリング中...これには数分かかることがあります。", "高解像度にアップスケーリング中...", "サウンドと最終仕上げを追加中...", "もうすぐです、最終的なビデオファイルを準備しています。" ],
    playerTitle: "生成が完了しました！",
    downloadButton: "ビデオをダウンロード",
    storyApiKeyManagerTitle: "ストーリー＆リサーチAPIキー",
    videoApiKeyManagerTitle: "プロフェッショナルビデオAPIキー",
    addNewStoryKeyLabel: "新しいストーリーAPIキーを追加",
    addNewVideoKeyLabel: "新しいプロフェッショナルビデオAPIキーを追加",
    apiKeyInputPlaceholder: "Gemini APIキーを入力してください",
    addKeyButton: "キーを追加",
    validatingButton: "検証中...",
    savedStoryKeysLabel: "保存されたストーリーキー",
    savedVideoKeysLabel: "保存されたプロフェッショナルビデオキー",
    noKeysSaved: "保存されているAPIキーはありません。",
    addKeyPrompt: "開始するには、上にキーを追加してください。",
    closeButton: "閉じる",
    errorKeyEmpty: "APIキーは空にできません。",
    errorKeyExists: "このAPIキーは既に追加されています。",
    errorKeyInvalid: "無効なAPIキーまたはネットワークの問題です。キーを確認して再試行してください。",
    generationFailed: "生成に失敗しました",
    alertEnterPrompt: "プロンプトを入力してください。",
    alertSetStoryApiKey: "生成する前に、アクティブなストーリーAPIキーを設定してください。",
    alertSetVideoApiKey: "ビデオを生成する前に、アクティブなプロフェッショナルビデオAPIキーを設定してください。",
    backToStoryCreator: "ストーリークリエーターに戻る",
    storyCreator: {
      newStory: "新しいストーリー",
      characterGarage: "キャラクターガレージ",
      garageDescription: "お気に入りのおもちゃをここに登録して、主役にしましょう！",
      openCharacterWorkshop: "キャラクターワークショップを開く",
      garageEmpty: "夢のガレージは空です。",
      directingDesk: "演出デスク",
      deskDescription: "ストーリーの追加設定。",
      sceneSet: "シーン設定:",
      locationSet: "主な場所の設定:",
      weatherSet: "天気の設定:",
      cameraStyleSet: "カメラスタイル (POV):",
      narratorLanguageSet: "ナレーター言語設定:",
      customLocationPlaceholder: "例：巨大なキッチンのジオラマ",
      customLanguagePlaceholder: "例：関西弁",
      storyEditor: "ストーリーエディター",
      storyboard: "ストーリーボード",
      needIdea: "ストーリーのアイデアが必要ですか？",
      ideaDescription: "スマートディレクターを使って、すぐに放送できる脚本の概要を作成しましょう！",
      openSmartDirector: "スマートディレクターを開く",
      storyTitle: "ストーリーのタイトル:",
      storyTitlePlaceholder: "例：赤いレーシングカーのリノと勇敢なモンスタートラックのゴロ",
      storyScript: "脚本/概要:",
      storyScriptPlaceholder: "ここにストーリーの概要や主なアイデアを書いてください。AIがそれをプロのシーンに展開します。",
      sceneCount: "シーン数:",
      createStoryboard: "ストーリーボードを作成！",
      storyboardPlaceholderTitle: "ストーリーの結果はここに表示されます",
      storyboardPlaceholderDescription: "「ストーリーボードを作成！」をクリックして開始します。",
      scene: "シーン",
      cinematography: "撮影技術",
      soundEffects: "効果音",
      noSfx: "SFXの提案はありません。",
      mixingGuide: "スマートミキシングガイド",
      generateBlueprint: "ブループリントを設計",
      generateCinematicPrompt: "シネマティックプロンプトを作成",
      useThisPrompt: "このプロンプトでビデオを生成",
      resultBlueprint: "結果：ブループリント設計",
      resultCinematic: "結果：シネマティックプロンプト",
      error: "エラー",
      confirmNewStoryTitle: "新しいストーリーを開始しますか？",
      confirmNewStoryMessage: "現在の進捗はすべて削除されます。よろしいですか？",
      directingOptions: {
        sceneSet: { standard_cinematic: "標準的なシネマティックアドベンチャー", epic_destruction: "壮大な破壊（スローモーション）", drifting_precision: "ドリフト精度チャレンジ" },
        locationSet: { standardLandGroup: "🏞️ 標準＆陸上セット", natural_outdoor: "屋外（庭/庭）", kids_bedroom: "子供部屋", custom_location: "独自の場所を入力..." },
        weatherSet: { sunny: "晴れ", cloudy: "曇り", rainy: "雨" },
        cameraStyleSet: { standardGroup: "🎥 標準スタイル", standard_cinematic: "標準シネマティック", fpv_drone_dive: "FPVドローンダイブ" },
        narratorLanguageSet: { no_narrator: "ナレーターなし", id: "インドネシア語", en: "英語", custom_language: "独自の言語を入力..." }
      }
    },
    characterWorkshop: {
      titleCreate: "AIキャラクターワークショップ",
      titleEdit: "キャラクターを編集",
      step1Title: "ステップ1/3：画像をアップロードするか、おもちゃのアイデアを説明してください。",
      imagePreview: "画像プレビュー",
      uploadButton: "参照画像をアップロード",
      ideaPlaceholder: "メモや追加の説明を追加します（画像がアップロードされている場合はオプション）...",
      designButton: "ユニークなモデルをデザイン",
      designingButton: "デザイン中...",
      step2Title: "ステップ2/3：AIが生成したモデルの詳細を確認・編集します。",
      brandName: "架空のブランド名:",
      modelName: "特定のモデル名:",
      consistencyId: "一貫性ID（ユニークトークン）:",
      consistencyIdHint: "キャラクターの一貫性を保つために、すべてのプロンプトでこのユニークIDを使用してください。",
      mainMaterial: "主な素材:",
      designLanguage: "ブランドデザイン言語:",
      keyFeatures: "主な特徴（視覚的DNA）:",
      step3Title: "ステップ3/3：このおもちゃの「アクションDNA」を定義します。",
      actionDnaSuggestions: "AIアクションDNAの提案:",
      addCustomAction: "カスタムアクションを追加:",
      customActionPlaceholder: "例：水上スキミング",
      addButton: "追加",
      selectedActionDna: "選択されたアクションDNA:",
      nextButton: "次へ",
      backButton: "戻る",
      saveButton: "キャラクターを保存",
      updateButton: "キャラクターを更新",
      alertUploadOrDescribe: "画像をアップロードするか、おもちゃのアイデアを説明してください。",
      alertRequiredFields: "ブランド名、モデル名、一貫性IDは必須です。"
    },
    smartDirector: {
      title: "スマートディレクター",
      step1Description: "すぐに放送できる脚本の概要を作成しましょう！簡単な手順に従ってください。",
      step1Label: "ステップ1：コンテンツ形式を選択",
      step2Label: "ステップ2：主人公を選択",
      step3Label: "ステップ3：ストーリーのテーマを選択",
      generateIdeasButton: "3つのアイデアをください！",
      generatingIdeasButton: "考え中...",
      step2Title: "お気に入りの脚本のアイデアを選択してください！",
      tryAgainButton: "↻ 新しいアイデアをリクエスト",
      applyIdeaButton: "✅ このアイデアを使用する！",
      cancelButton: "キャンセル",
      contentFormats: {
        cinematic_adventure: "シネマティックアドベンチャー＆ストーリー",
        product_review: "製品レビュー",
        unboxing: "開封と第一印象",
        vs_challenge: "比較ビデオ（VSチャレンジ）",
      },
      characterOptions: {
        random: "ランダムに選択",
        yourGarage: "🚗 あなたのガレージ",
        construction: "🚛 建設車両",
        beniBulldozer: "勇敢なブルドーザーのベニ",
      },
      themeOptions: {
        random: "ランダムに選択",
        adventureGroup: "アドベンチャー＆探検",
        explore_new_area: "新しいエリアの探検",
        rescue_mission: "別のおもちゃの救出ミッション",
        challengeGroup: "物理的な挑戦と障害",
        overcome_obstacle: "巨大な障害を乗り越える",
        custom_theme: "独自のテーマを入力...",
      },
      customThemePlaceholder: "例：火星でのレース"
    }
  },
  de: {
    appName: "AL ANSHOR VEO GENERATOR",
    appTagline: "Erstellen Sie beeindruckende Videos aus Text oder Bildern.",
    manageStoryApiKeys: "Story API-Schlüssel",
    manageVideoApiKeys: "Video API-Schlüssel",
    promptLabel: "Prompt",
    promptPlaceholder: "Ein majestätischer Löwe, der bei Sonnenuntergang die Savanne überblickt...",
    promptHint: "Sie können einfachen Text oder eine JSON-formatierte Zeichenfolge verwenden.",
    referenceImageLabel: "Referenzbild (Optional)",
    uploadFile: "Datei hochladen",
    dragAndDrop: "oder ziehen und ablegen",
    fileTypes: "PNG, JPG, GIF bis zu 10MB",
    generationSettings: "Generierungseinstellungen",
    aspectRatioLabel: "Seitenverhältnis",
    soundLabel: "Ton",
    enableSound: "Ton aktivieren",
    resolutionLabel: "Auflösung",
    videoKeyMissingWarning: "Bitte fügen Sie einen professionellen Video-API-Schlüssel hinzu, um zu beginnen.",
    generateButton: "Video generieren",
    generatingButton: "Video wird generiert...",
    loaderTitle: "Ihr Video wird generiert",
    loadingMessages: [ "VEO-Modell wird initialisiert...", "Ihr Prompt wird analysiert...", "Hauptszene wird komponiert...", "Anfangsbilder werden generiert...", "Videosequenz wird gerendert... dies kann einige Minuten dauern.", "Hochskalieren auf hohe Auflösung...", "Ton und letzte Feinheiten werden hinzugefügt...", "Fast fertig, die endgültige Videodatei wird vorbereitet." ],
    playerTitle: "Generierung abgeschlossen!",
    downloadButton: "Video herunterladen",
    storyApiKeyManagerTitle: "Story & Recherche API-Schlüssel",
    videoApiKeyManagerTitle: "Professioneller Video-API-Schlüssel",
    addNewStoryKeyLabel: "Neuen Story API-Schlüssel hinzufügen",
    addNewVideoKeyLabel: "Neuen professionellen Video-API-Schlüssel hinzufügen",
    apiKeyInputPlaceholder: "Geben Sie Ihren Gemini-API-Schlüssel ein",
    addKeyButton: "Schlüssel hinzufügen",
    validatingButton: "Wird validiert...",
    savedStoryKeysLabel: "Gespeicherte Story-Schlüssel",
    savedVideoKeysLabel: "Gespeicherte professionelle Video-Schlüssel",
    noKeysSaved: "Keine API-Schlüssel gespeichert.",
    addKeyPrompt: "Fügen Sie oben einen Schlüssel hinzu, um zu beginnen.",
    closeButton: "Schließen",
    errorKeyEmpty: "API-Schlüssel darf nicht leer sein.",
    errorKeyExists: "Dieser API-Schlüssel wurde bereits hinzugefügt.",
    errorKeyInvalid: "Ungültiger API-Schlüssel oder Netzwerkproblem. Bitte überprüfen Sie den Schlüssel und versuchen Sie es erneut.",
    generationFailed: "Generierung fehlgeschlagen",
    alertEnterPrompt: "Bitte geben Sie einen Prompt ein.",
    alertSetStoryApiKey: "Bitte legen Sie vor dem Generieren einen aktiven Story-API-Schlüssel fest.",
    alertSetVideoApiKey: "Bitte legen Sie vor dem Generieren eines Videos einen aktiven professionellen Video-API-Schlüssel fest.",
    backToStoryCreator: "Zurück zum Story Creator",
    storyCreator: {
      newStory: "Neue Geschichte",
      characterGarage: "Charakter-Garage",
      garageDescription: "Registrieren Sie hier Ihre Lieblingsspielzeuge, um sie zum Hauptdarsteller zu machen!",
      openCharacterWorkshop: "Charakter-Werkstatt öffnen",
      garageEmpty: "Ihre Traumgarage ist leer.",
      directingDesk: "Regiepult",
      deskDescription: "Zusätzliche Einstellungen für Ihre Geschichte.",
      sceneSet: "Szenen-Set:",
      locationSet: "Hauptort-Set:",
      weatherSet: "Wetter-Set:",
      cameraStyleSet: "Kamerastil (POV):",
      narratorLanguageSet: "Erzählersprache:",
      customLocationPlaceholder: "z.B. Riesiges Küchen-Diorama",
      customLanguagePlaceholder: "z.B. Bayerisch",
      storyEditor: "Story-Editor",
      storyboard: "Storyboard",
      needIdea: "Brauchen Sie eine Story-Idee?",
      ideaDescription: "Nutzen Sie den Smart Director, um einen sendefertigen Drehbuchentwurf zu erstellen!",
      openSmartDirector: "Smart Director öffnen",
      storyTitle: "Titel der Geschichte:",
      storyTitlePlaceholder: "z.B. Rino, der rote Rennwagen und Goro, der tapfere Monstertruck",
      storyScript: "Drehbuch / Zusammenfassung:",
      storyScriptPlaceholder: "Schreiben Sie hier eine Zusammenfassung der Geschichte oder die Hauptidee. Die KI wird sie zu professionellen Szenen ausarbeiten.",
      sceneCount: "Anzahl der Szenen:",
      createStoryboard: "Storyboard erstellen!",
      storyboardPlaceholderTitle: "Ihre Story-Ergebnisse werden hier angezeigt",
      storyboardPlaceholderDescription: "Klicken Sie auf 'Storyboard erstellen!', um zu beginnen.",
      scene: "Szene",
      cinematography: "Kameraführung",
      soundEffects: "Soundeffekte",
      noSfx: "Keine SFX-Vorschläge.",
      mixingGuide: "Intelligente Mischanleitung",
      generateBlueprint: "Blaupause entwerfen",
      generateCinematicPrompt: "Kinoreifen Prompt erstellen",
      useThisPrompt: "Video mit diesem Prompt generieren",
      resultBlueprint: "Ergebnis: Blaupausen-Entwurf",
      resultCinematic: "Ergebnis: Kinoreifer Prompt",
      error: "Fehler",
      confirmNewStoryTitle: "Eine neue Geschichte beginnen?",
      confirmNewStoryMessage: "Alle aktuellen Fortschritte werden gelöscht. Sind Sie sicher?",
      directingOptions: {
        sceneSet: { standard_cinematic: "Standard-Kinoabenteuer", epic_destruction: "Epische Zerstörung (Zeitlupe)", drifting_precision: "Drift-Präzisions-Herausforderung" },
        locationSet: { standardLandGroup: "🏞️ Standard- & Land-Sets", natural_outdoor: "Im Freien (Garten/Hof)", kids_bedroom: "Kinderzimmer", custom_location: "Eigenen Ort eingeben..." },
        weatherSet: { sunny: "Sonnig", cloudy: "Bewölkt", rainy: "Regnerisch" },
        cameraStyleSet: { standardGroup: "🎥 Standard-Stile", standard_cinematic: "Standard-Kino", fpv_drone_dive: "FPV-Drohnen-Tauchflug" },
        narratorLanguageSet: { no_narrator: "Ohne Erzähler", id: "Indonesisch", en: "Englisch", custom_language: "Eigene Sprache eingeben..." }
      }
    },
    characterWorkshop: {
      titleCreate: "KI-Charakter-Werkstatt",
      titleEdit: "Charakter bearbeiten",
      step1Title: "Schritt 1 von 3: Laden Sie ein Bild hoch oder beschreiben Sie Ihre Spielzeugidee.",
      imagePreview: "Bildvorschau",
      uploadButton: "Referenzbild hochladen",
      ideaPlaceholder: "Fügen Sie Notizen oder eine zusätzliche Beschreibung hinzu (optional, wenn ein Bild hochgeladen wird)...",
      designButton: "Einzigartiges Modell entwerfen",
      designingButton: "Entwerfen...",
      step2Title: "Schritt 2 von 3: Überprüfen und bearbeiten Sie die von der KI generierten Modelldetails.",
      brandName: "Fiktiver Markenname:",
      modelName: "Spezifischer Modellname:",
      consistencyId: "Konsistenz-ID (Einzigartiger Token):",
      consistencyIdHint: "Verwenden Sie diese einzigartige ID in jedem Prompt, um die Charakterkonsistenz zu wahren.",
      mainMaterial: "Hauptmaterial:",
      designLanguage: "Markendesignsprache:",
      keyFeatures: "Hauptmerkmale (Visuelle DNA):",
      step3Title: "Schritt 3 von 3: Definieren Sie die „Aktions-DNA“ dieses Spielzeugs.",
      actionDnaSuggestions: "KI-Aktions-DNA-Vorschläge:",
      addCustomAction: "Benutzerdefinierte Aktion hinzufügen:",
      customActionPlaceholder: "z.B. Wassergleiten",
      addButton: "Hinzufügen",
      selectedActionDna: "Ausgewählte Aktions-DNA:",
      nextButton: "Weiter",
      backButton: "Zurück",
      saveButton: "Charakter speichern",
      updateButton: "Charakter aktualisieren",
      alertUploadOrDescribe: "Bitte laden Sie ein Bild hoch oder beschreiben Sie Ihre Spielzeugidee.",
      alertRequiredFields: "Markenname, Modellname und Konsistenz-ID sind erforderlich."
    },
    smartDirector: {
      title: "Smart Director",
      step1Description: "Erstellen wir einen sendefertigen Drehbuchentwurf! Folgen Sie diesen einfachen Schritten.",
      step1Label: "Schritt 1: Inhaltsformat wählen",
      step2Label: "Schritt 2: Hauptfigur wählen",
      step3Label: "Schritt 3: Story-Thema wählen",
      generateIdeasButton: "Gib mir 3 Ideen!",
      generatingIdeasButton: "Denken...",
      step2Title: "Wählen Sie Ihre Lieblings-Drehbuchidee!",
      tryAgainButton: "↻ Neue Ideen anfordern",
      applyIdeaButton: "✅ Diese Idee verwenden!",
      cancelButton: "Abbrechen",
      contentFormats: {
        cinematic_adventure: "Kinoabenteuer & Geschichte",
        product_review: "Produktbewertung",
        unboxing: "Unboxing & Erste Eindrücke",
        vs_challenge: "Vergleichsvideo (VS-Herausforderung)",
      },
      characterOptions: {
        random: "Zufällig wählen",
        yourGarage: "🚗 Deine Garage",
        construction: "🚛 Baufahrzeuge",
        beniBulldozer: "Beni der tapfere Bulldozer",
      },
      themeOptions: {
        random: "Zufällig wählen",
        adventureGroup: "Abenteuer & Erkundung",
        explore_new_area: "Ein neues Gebiet erkunden",
        rescue_mission: "Rettungsmission für ein anderes Spielzeug",
        challengeGroup: "Körperliche Herausforderungen & Hindernisse",
        overcome_obstacle: "Ein riesiges Hindernis überwinden",
        custom_theme: "Eigenes Thema eingeben...",
      },
      customThemePlaceholder: "z.B. Rennen auf dem Mars"
    }
  },
  fr: {
    appName: "GÉNÉRATEUR VIDÉO AL ANSHOR VEO",
    appTagline: "Générez des vidéos époustouflantes à partir de texte ou d'images.",
    manageStoryApiKeys: "Clé API Histoire",
    manageVideoApiKeys: "Clé API Vidéo",
    promptLabel: "Instruction",
    promptPlaceholder: "Un lion majestueux surplombant la savane au coucher du soleil...",
    promptHint: "Vous pouvez utiliser du texte brut ou une chaîne au format JSON.",
    referenceImageLabel: "Image de référence (Optionnel)",
    uploadFile: "Télécharger un fichier",
    dragAndDrop: "ou glisser-déposer",
    fileTypes: "PNG, JPG, GIF jusqu'à 10 Mo",
    generationSettings: "Paramètres de génération",
    aspectRatioLabel: "Ratio d'aspect",
    soundLabel: "Son",
    enableSound: "Activer le son",
    resolutionLabel: "Résolution",
    videoKeyMissingWarning: "Veuillez ajouter une clé API vidéo professionnelle pour commencer.",
    generateButton: "Générer la vidéo",
    generatingButton: "Génération de la vidéo...",
    loaderTitle: "Génération de votre vidéo",
    loadingMessages: [ "Initialisation du modèle VEO...", "Analyse de votre instruction...", "Composition de la scène principale...", "Génération des premières images...", "Rendu de la séquence vidéo... cela peut prendre quelques minutes.", "Mise à l'échelle en haute résolution...", "Ajout du son et des touches finales...", "Presque terminé, préparation du fichier vidéo final." ],
    playerTitle: "Génération terminée !",
    downloadButton: "Télécharger la vidéo",
    storyApiKeyManagerTitle: "Clé API Histoire & Recherche",
    videoApiKeyManagerTitle: "Clé API Vidéo Professionnelle",
    addNewStoryKeyLabel: "Ajouter une nouvelle clé API Histoire",
    addNewVideoKeyLabel: "Ajouter une nouvelle clé API Vidéo Professionnelle",
    apiKeyInputPlaceholder: "Entrez votre clé API Gemini",
    addKeyButton: "Ajouter la clé",
    validatingButton: "Validation...",
    savedStoryKeysLabel: "Clés d'histoire enregistrées",
    savedVideoKeysLabel: "Clés vidéo professionnelles enregistrées",
    noKeysSaved: "Aucune clé API enregistrée.",
    addKeyPrompt: "Ajoutez une clé ci-dessus pour commencer.",
    closeButton: "Fermer",
    errorKeyEmpty: "La clé API ne peut pas être vide.",
    errorKeyExists: "Cette clé API a déjà été ajoutée.",
    errorKeyInvalid: "Clé API invalide ou problème de réseau. Veuillez vérifier la clé et réessayer.",
    generationFailed: "Échec de la génération",
    alertEnterPrompt: "Veuillez entrer une instruction.",
    alertSetStoryApiKey: "Veuillez définir une clé API d'histoire active avant la génération.",
    alertSetVideoApiKey: "Veuillez définir une clé API vidéo professionnelle active avant de générer une vidéo.",
    backToStoryCreator: "Retour au créateur d'histoires",
    storyCreator: {
      newStory: "Nouvelle histoire",
      characterGarage: "Garage des personnages",
      garageDescription: "Enregistrez ici vos jouets préférés pour en faire la vedette !",
      openCharacterWorkshop: "Ouvrir l'atelier des personnages",
      garageEmpty: "Le garage de vos rêves est vide.",
      directingDesk: "Bureau de réalisation",
      deskDescription: "Paramètres supplémentaires pour votre histoire.",
      sceneSet: "Ensemble de scènes :",
      locationSet: "Lieu principal :",
      weatherSet: "Météo :",
      cameraStyleSet: "Style de caméra (POV) :",
      narratorLanguageSet: "Langue du narrateur :",
      customLocationPlaceholder: "Ex: Diorama de cuisine géante",
      customLanguagePlaceholder: "Ex: Alsacien",
      storyEditor: "Éditeur d'histoire",
      storyboard: "Storyboard",
      needIdea: "Besoin d'une idée d'histoire ?",
      ideaDescription: "Utilisez le Réalisateur Intelligent pour créer une ébauche de scénario prête à être diffusée !",
      openSmartDirector: "Ouvrir le Réalisateur Intelligent",
      storyTitle: "Titre de l'histoire :",
      storyTitlePlaceholder: "Ex: Rino la voiture de course rouge et Goro le courageux camion monstre",
      storyScript: "Scénario / Résumé de l'histoire :",
      storyScriptPlaceholder: "Écrivez un résumé de l'histoire ou l'idée principale ici. L'IA la développera en scènes professionnelles.",
      sceneCount: "Nombre de scènes :",
      createStoryboard: "Créer le storyboard !",
      storyboardPlaceholderTitle: "Les résultats de votre histoire apparaîtront ici",
      storyboardPlaceholderDescription: "Cliquez sur 'Créer le storyboard !' pour commencer.",
      scene: "Scène",
      cinematography: "Cinématographie",
      soundEffects: "Effets sonores",
      noSfx: "Aucune suggestion d'effets sonores.",
      mixingGuide: "Guide de mixage intelligent",
      generateBlueprint: "Concevoir le plan",
      generateCinematicPrompt: "Créer une instruction cinématique",
      useThisPrompt: "Générer une vidéo avec cette instruction",
      resultBlueprint: "Résultat : Conception du plan",
      resultCinematic: "Résultat : Instruction cinématique",
      error: "Erreur",
      confirmNewStoryTitle: "Commencer une nouvelle histoire ?",
      confirmNewStoryMessage: "Toute la progression actuelle sera supprimée. Êtes-vous sûr ?",
      directingOptions: {
        sceneSet: { standard_cinematic: "Aventure cinématique standard", epic_destruction: "Destruction épique (ralenti)", drifting_precision: "Défi de précision de dérapage" },
        locationSet: { standardLandGroup: "🏞️ Ensembles standard et terrestres", natural_outdoor: "Extérieur (Jardin/Cour)", kids_bedroom: "Chambre d'enfant", custom_location: "Entrez votre propre lieu..." },
        weatherSet: { sunny: "Ensoleillé", cloudy: "Nuageux", rainy: "Pluvieux" },
        cameraStyleSet: { standardGroup: "🎥 Styles standard", standard_cinematic: "Cinématique standard", fpv_drone_dive: "Plongée de drone FPV" },
        narratorLanguageSet: { no_narrator: "Sans narrateur", id: "Indonésien", en: "Anglais", custom_language: "Entrez votre propre langue..." }
      }
    },
    characterWorkshop: {
      titleCreate: "Atelier de personnages IA",
      titleEdit: "Modifier le personnage",
      step1Title: "Étape 1 sur 3 : Téléchargez une image ou décrivez votre idée de jouet.",
      imagePreview: "Aperçu de l'image",
      uploadButton: "Télécharger une image de référence",
      ideaPlaceholder: "Ajoutez des notes ou une description supplémentaire (facultatif si l'image est téléchargée)...",
      designButton: "Concevoir un modèle unique",
      designingButton: "Conception...",
      step2Title: "Étape 2 sur 3 : Révisez et modifiez les détails du modèle générés par l'IA.",
      brandName: "Nom de marque fictif :",
      modelName: "Nom de modèle spécifique :",
      consistencyId: "ID de cohérence (Jeton unique) :",
      consistencyIdHint: "Utilisez cet ID unique dans chaque instruction pour maintenir la cohérence du personnage.",
      mainMaterial: "Matériau principal :",
      designLanguage: "Langage de conception de la marque :",
      keyFeatures: "Caractéristiques clés (ADN visuel) :",
      step3Title: "Étape 3 sur 3 : Définissez l'« ADN d'action » de ce jouet.",
      actionDnaSuggestions: "Suggestions d'ADN d'action de l'IA :",
      addCustomAction: "Ajouter une action personnalisée :",
      customActionPlaceholder: "Ex: Glisse sur l'eau",
      addButton: "Ajouter",
      selectedActionDna: "ADN d'action sélectionné :",
      nextButton: "Suivant",
      backButton: "Retour",
      saveButton: "Enregistrer le personnage",
      updateButton: "Mettre à jour le personnage",
      alertUploadOrDescribe: "Veuillez télécharger une image ou décrire votre idée de jouet.",
      alertRequiredFields: "Le nom de la marque, le nom du modèle et l'ID de cohérence sont requis."
    },
    smartDirector: {
      title: "Réalisateur Intelligent",
      step1Description: "Créons une ébauche de scénario prête à être diffusée ! Suivez ces étapes faciles.",
      step1Label: "Étape 1 : Choisissez le format du contenu",
      step2Label: "Étape 2 : Choisissez le personnage principal",
      step3Label: "Étape 3 : Choisissez le thème de l'histoire",
      generateIdeasButton: "Donnez-moi 3 idées !",
      generatingIdeasButton: "Réflexion...",
      step2Title: "Choisissez votre idée de scénario préférée !",
      tryAgainButton: "↻ Demander de nouvelles idées",
      applyIdeaButton: "✅ Utiliser cette idée !",
      cancelButton: "Annuler",
      contentFormats: {
        cinematic_adventure: "Aventure et histoire cinématiques",
        product_review: "Critique de produit",
        unboxing: "Déballage et premières impressions",
        vs_challenge: "Vidéo de comparaison (défi VS)",
      },
      characterOptions: {
        random: "Choisir au hasard",
        yourGarage: "🚗 Votre garage",
        construction: "🚛 Véhicules de construction",
        beniBulldozer: "Beni le courageux bulldozer",
      },
      themeOptions: {
        random: "Choisir au hasard",
        adventureGroup: "Aventure et exploration",
        explore_new_area: "Explorer une nouvelle zone",
        rescue_mission: "Mission de sauvetage pour un autre jouet",
        challengeGroup: "Défis et obstacles physiques",
        overcome_obstacle: "Surmonter un obstacle géant",
        custom_theme: "Entrez votre propre thème...",
      },
      customThemePlaceholder: "Ex: Course sur la planète Mars"
    }
  },
};

interface LanguageContextType {
  language: Language;
  setLanguage: (lang: Language) => void;
  t: (key: string) => string | string[];
  dir: 'ltr' | 'rtl';
}

const LanguageContext = createContext<LanguageContextType | undefined>(undefined);

const getInitialLanguage = (): Language => {
    try {
        const storedLang = localStorage.getItem(LANGUAGE_STORAGE_KEY);
        if (storedLang && Object.keys(languageMap).includes(storedLang)) {
            return storedLang as Language;
        }
        const browserLang = navigator.language.split('-')[0];
        if (browserLang && Object.keys(languageMap).includes(browserLang)) {
           return browserLang as Language;
        }
    } catch (e) {
        console.error("Could not access localStorage, defaulting to English.", e);
    }
    return 'id'; // Default to Indonesian
};

export const LanguageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [language, setLanguageState] = useState<Language>(getInitialLanguage);

  useEffect(() => {
    try {
        localStorage.setItem(LANGUAGE_STORAGE_KEY, language);
    } catch (e) {
        console.error("Could not save language to localStorage.", e);
    }
  }, [language]);

  const setLanguage = (lang: Language) => {
    setLanguageState(lang);
  };
  
  const t = useCallback((key: string): string | string[] => {
    const keys = key.split('.');

    const getTranslation = (lang: Language): string | string[] | Translations | undefined => {
        let res: any = translations[lang];
        for (const k of keys) {
            res = res?.[k];
        }
        return res;
    };

    const result = getTranslation(language) ?? getTranslation('en') ?? getTranslation('id');

    if (result === undefined) {
        return key;
    }

    if (typeof result === 'object' && result !== null && !Array.isArray(result)) {
        console.warn(`Translation for key '${key}' resolves to an object, which cannot be rendered. Returning the key itself.`);
        return key;
    }
    
    return result as string | string[];
  }, [language]);

  const dir = language === 'ar' ? 'rtl' : 'ltr';

  return (
    <LanguageContext.Provider value={{ language, setLanguage, t, dir }}>
      {children}
    </LanguageContext.Provider>
  );
};

export const useLocalization = (): LanguageContextType => {
  const context = useContext(LanguageContext);
  if (!context) {
    throw new Error('useLocalization must be used within a LanguageProvider');
  }
  return context;
};